...
слизистую и **разворачивает её в плоскую карту** в истинном масштабе —
по такой карте хирург измеряет дефект и подбирает лоскут, не пересчитывая
размеры с изогнутой поверхности в уме. Видео работы — в релизе 3.
...
Чекпойнт раздаётся через [Releases](../../releases) — в репозиторий
он не входит из-за размера.

## Запуск

При первом старте на вкладке «Разметка КТ» откройте «Настройки модели»
и укажите три пути: интерпретатор, `infer_swinunetr_cli.py` и файл
чекпойнта. Они запомнятся.
