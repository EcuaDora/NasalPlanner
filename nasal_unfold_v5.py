...
def orient_faces_coherently(V, F, verbose=False):
    """Согласовать обход граней (ориентацию нормалей) обходом в ширину.

    Зачем. У согласованной поверхности каждое внутреннее ребро проходится
    соседними гранями в ПРОТИВОПОЛОЖНЫХ направлениях: (a,b) у одной и
    (b,a) у другой. Если направление совпадает, нормали смотрят в разные
    стороны, и поверхность формально НЕОРИЕНТИРУЕМА — как лента Мёбиуса.

    Чем это ломает развёртку. Формула Эйлера chi = 2 - 2g - b перестаёт
    давать целый род: на реальном меше выходило g = 7.5, чего не бывает.
    cut_handles() ищет «ручки», которых нет, режет впустую и не может
    добраться до диска. LSCM на такой поверхности даёт складки.

    Откуда берётся. pymeshlab пытается чинить это фильтром
    meshing_re_orient_faces_coherently, но пропускает его, если в меше
    есть не-2-манифолдные грани — в логе препроцесса так и написано:
    «Orientability requires manifoldness». Меш идёт дальше сломанным.

    Что делает функция. Обход в ширину по графу смежности граней: сосед
    получает согласованный обход, при необходимости грань переворачивается.
    Геометрия НЕ меняется — только порядок индексов внутри треугольника,
    площади и координаты остаются теми же.
    """
    nF = len(F)
    if nF == 0:
        return F, 0
    ...
    for fi in range(nF):
        ...
        for x, y in ((a, b), (b, c), (c, a)):
            e2f[(x, y) if x < y else (y, x)].append(fi)
    ...
    for root in range(nF):
        ...
        while dq:
            ...
            for x, y in ((a, b), (b, c), (c, a)):
                ...
                fl = e2f.get(key, ())
                if len(fl) != 2:
                    continue                    # граница или non-manifold
                fj = fl[0] if fl[1] == fi else fl[1]
                if seen[fj]:
                    continue
                seen[fj] = True
                # у соседа то же ребро должно идти как (y, x)
                p, q, r = int(F[fj, 0]), int(F[fj, 1]), int(F[fj, 2])
                same = ((p, q) == (x, y) or (q, r) == (x, y) or (r, p) == (x, y))
                if same:                        # совпало — переворачиваем
                    F[fj, 1], F[fj, 2] = F[fj, 2], F[fj, 1]
                    flipped += 1
                dq.append(fj)

    if verbose and flipped:
        print(f"    [orient] согласовано граней: {flipped} из {nF}")
    return F, flipped


def _find_handle_loops(V, F, max_loops=8):
    """Найти нестягиваемые петли («ручки») алгоритмом tree-cotree.

    Зачем. Для развёртки нужен топологический ДИСК: chi = 1, одна граница,
    род 0. Поверхность рода g нельзя разложить на плоскость без наложений —
    сколько ни оптимизируй ARAP, часть площади ляжет сама на себя. Чтобы
    убрать ручку, её нужно разрезать по нестягиваемой петле; на каждую
    ручку требуется один такой разрез.

    Откуда ручки берутся на слизистой. Типичный источник — соприкосновение
    перегородки с латеральной стенкой (синехия, шип, контакт с раковиной).
    В месте контакта поверхность замыкается в кольцо, и просвет вокруг него
    становится «тоннелем» — топологически это и есть ручка.

    Алгоритм (Eppstein, tree-cotree decomposition):
      1. T  — остовное дерево вершинного графа.
      2. C  — остовное дерево двойственного графа (граней), из рёбер,
              не попавших в T.
      3. Рёбра, не вошедшие ни в T, ни в C, — образующие. Их ровно 2g
         для замкнутой поверхности рода g; при наличии границы часть
         вырождается, поэтому итог проверяем по chi.
      4. Петля для образующего ребра (a, b) = путь в T от a до b плюс
         само ребро.

    Возвращает список путей [v0, ..., vk] (замкнутых: v0 == vk),
    отсортированных по длине — короткие режем первыми, они дают
    наименьшее искажение.
    """
    nV, nF = len(V), len(F)
    if nF == 0:
        return []

    # ── рёбра и смежность граней ────────────────────────────────
    edge2face = {}
    for fi in range(nF):
        a, b, c = int(F[fi, 0]), int(F[fi, 1]), int(F[fi, 2])
        for x, y in ((a, b), (b, c), (c, a)):
    ...
    # ── 1. остовное дерево вершинного графа T ───────────────────
    vadj = [[] for _ in range(nV)]
    for (a, b) in edge2face:
        vadj[a].append(b)
        vadj[b].append(a)

    in_tree = set()                 # рёбра дерева T
    parent = [-1] * nV
    seen_v = [False] * nV
    from collections import deque
    for root in range(nV):
        if seen_v[root] or not vadj[root]:
            continue
        seen_v[root] = True
        dq = deque([root])
        while dq:
            ...
            for w in vadj[v]:
                ...
                seen_v[w] = True
                parent[w] = v
                in_tree.add((v, w) if v < w else (w, v))
                dq.append(w)

    # ── 2. остовное дерево двойственного графа C ────────────────
    # Идём по граням; ребро годится, если оно НЕ в T и внутреннее.
    in_cotree = set()
    seen_f = [False] * nF
    for root in range(nF):
        if seen_f[root]:
        ...
        while dq:
            ...
            for x, y in ((a, b), (b, c), (c, a)):
                ...
                if e in in_tree:
                    continue
                fl = edge2face.get(e, ())
                if len(fl) != 2:            # граничное или non-manifold
                    continue
                fj = fl[0] if fl[1] == fi else fl[1]
                if seen_f[fj]:
                    continue
    ...
    # ── 3. образующие: ни в T, ни в C ───────────────────────────
    gens = [e for e in edge2face
            if e not in in_tree and e not in in_cotree
            and len(edge2face[e]) == 2]
    if not gens:
        return []

    # ── 4. путь ГРАНИЦА → образующая → ГРАНИЦА ──────────────────
    # Замкнутая петля вокруг ручки для _apply_cut не годится: fan-split
    # раскрывает разрез только там, где у пути есть концы на границе.
    # Поэтому берём образующее ребро и тянем от обоих его концов
    # кратчайшие пути до границы — получается путь boundary→boundary,
    # проходящий «сквозь» ручку. Разрез по нему поднимает chi на 1.
    bnd_verts = set()
    ...
    def path_to_boundary(src):
        """Кратчайший путь по рёбрам от src до ближайшей граничной вершины."""
        if src in bnd_verts:
            return [src]
        prev = {src: -1}
        dq = deque([src])
    ...
    loops = []
    for (a, b) in gens:
        pa = path_to_boundary(a)
        pb = path_to_boundary(b)
        if not pa or not pb:
            continue
        # pa: a → граница; разворачиваем, чтобы путь шёл граница → a → b → граница
        path = pa[::-1] + pb
        ...
        for v in path[1:]:
            if v != clean[-1]:
                clean.append(v)
        # путь не должен самопересекаться — иначе fan-split запутается
...
def _path_between_boundaries(V, F, protected=None):
    """Кратчайший путь по рёбрам между ДВУМЯ РАЗНЫМИ граничными циклами.

    Нужен после разреза ручки: разрез раскрывает поверхность, но создаёт
    вторую границу, и chi поднимается только до 0. Диск получится, когда
    границы сольются в одну — для этого их надо соединить разрезом.

    protected — вершины, которые ТРОГАТЬ НЕЛЬЗЯ. Это края сохранённых
    перфораций.

    Зачем запрет. Перфорация — не дефект меша, а анатомия: врач меряет
    её диаметры и по ним подбирает лоскут. Разрез от внешнего края
    до перфорации ВСКРЫВАЕТ её: замкнутое отверстие превращается в
    прорезь, диаметры теряют смысл, край на карте расходится.

    Так и случилось: в отчёте развёртки было
        n_preserved_perforations: 1
        handle_cuts: [{'kind': 'join_boundaries', ...}]
    то есть перфорацию пометили как сохраняемую — и тут же разрезали.

    Лучше оставить chi = 0 и складку на развёртке, чем испортить
    перфорацию: складка видна и о ней предупреждают, а вскрытая
    перфорация выглядит достоверно и врёт.
    """
    ...
    for e, fl in e2f.items():
        if len(fl) != 1:
            continue
        badj.setdefault(e[0], []).append(e[1])
        badj.setdefault(e[1], []).append(e[0])
    comp = {}
    cid = 0
    for v in badj:
        if v in comp or v in protected:
            continue
        dq = deque([v]); comp[v] = cid
        while dq:
            x = dq.popleft()
            for y in badj.get(x, ()):
                if y not in comp:
    ...
    # BFS от самого большого цикла до любой вершины другого
    sizes = {}
    for v, c in comp.items():
        sizes[c] = sizes.get(c, 0) + 1
    main = max(sizes, key=lambda c: sizes[c])
    src = [v for v, c in comp.items() if c == main and v not in protected]
    if not src:
        return None
    prev = {v: -1 for v in src}
    dq = deque(src)
    while dq:
        v = dq.popleft()
        for w in vadj[v]:
            ...
            prev[w] = v
            if w in protected:
                continue                      # к перфорации не подходим
            if w in comp and comp[w] != main:
                out = [w]
                while prev[out[-1]] != -1:
                    out.append(prev[out[-1]])
                return out[::-1]
...
def cut_handles(V, F, verbose=False, protected_loops=None):
    """Разрезать все ручки, пока поверхность не станет диском (chi == 1).

    Режем по одной петле за раз и после каждого разреза пересчитываем chi:
    один разрез может убрать сразу несколько образующих, а может и ни
    одной (если петля оказалась стягиваемой). Останавливаемся, как только
    chi == 1 либо петли кончились.

    Возвращает (V, F, n_cuts, info, new_vert_parents_all).
    """
    def _chi(Vx, Fx):
        ed = np.sort(np.vstack([Fx[:, [0, 1]], Fx[:, [1, 2]], Fx[:, [2, 0]]]), axis=1)
        nE = len(set(map(tuple, ed)))
        return len(Vx) - nE + len(Fx)

    # Ориентацию чиним ДО подсчёта chi: на несогласованном меше формула
    # Эйлера даёт нецелый род, и поиск ручек уходит в пустоту.
    F, n_flipped = orient_faces_coherently(V, F, verbose=verbose)

    chi0 = _chi(V, F)
    if chi0 >= 1:
        return V, F, 0, [], []

    # Края сохранённых перфораций: их нельзя ни разрезать, ни использовать
    # как конец пути. Индексы приходят из classify_and_fill_inner_loops(),
    # который уже решил, что перфорация анатомическая, а не артефакт.
    protected = set()
    for loop in (protected_loops or []):
        for v in loop:
            protected.add(int(v))
    if protected and verbose:
        print(f"    [handle] под защитой {len(protected)} вершин "
              f"({len(protected_loops)} перфораций) — резать их нельзя")

    info, parents_all = [], []
    n_cuts = 0
    # Потолок 8 был рассчитан на род 1 (одна ручка). На шумной маске
    # встречается род 5-8: мелкие перемычки в вокселях становятся
    # тоннелями. Каждая ручка требует своего разреза, поэтому 32.
    # Это страховка от зацикливания, а не рабочий предел: цикл выходит
    # сам, как только chi достигает 1 или петли кончаются.
    for _ in range(32):
        loops = _find_handle_loops(V, F, max_loops=4)
        if not loops:
            break
        chi_before = _chi(V, F)
        applied = False
        for loop in loops:
            if protected and any(int(v) in protected for v in loop):
                continue                      # петля задевает перфорацию
            V2, F2, ndup, parents = _apply_cut(V, F, loop)
            chi_after = _chi(V2, F2)
            if chi_after > chi_before:       # разрез сработал
                V, F = V2, F2
                parents_all.extend(parents)
                n_cuts += 1
                info.append({'loop_len': len(loop), 'duplicated': int(ndup),
                             'chi_before': int(chi_before), 'chi_after': int(chi_after)})
                if verbose:
                    print(f"    [handle] разрез по петле из {len(loop)} вершин: "
                          f"chi {chi_before} -> {chi_after}, +{ndup} вершин")
                applied = True
                break
        if not applied:
            if verbose:
                print("    [handle] ни одна петля не изменила chi — останов")
            break
        if _chi(V, F) >= 1:
            break

    # Разрез ручки поднимает chi до 0 и оставляет вторую границу.
    # Соединяем границы, пока не получится диск (chi == 1).
    # Каждый разрез ручки добавляет границу, поэтому их может накопиться
    # столько же, сколько было ручек, плюс исходные.
    for _ in range(32):
        if _chi(V, F) >= 1:
            break
        path = _path_between_boundaries(V, F, protected)
        if not path or len(path) < 2:
            break
        chi_before = _chi(V, F)
        V2, F2, ndup, parents = _apply_cut(V, F, path)
        chi_after = _chi(V2, F2)
        ...
        parents_all.extend(parents)
        n_cuts += 1
        info.append({'loop_len': len(path), 'duplicated': int(ndup),
                     'chi_before': int(chi_before), 'chi_after': int(chi_after),
                     'kind': 'join_boundaries'})
        if verbose:
            print(f"    [handle] соединение границ по пути из {len(path)} вершин: "
                  f"chi {chi_before} -> {chi_after}")

    # Честный итог: если диск так и не получился, LSCM даст складки,
    # и об этом должно быть известно выше по стеку, а не «само как-то».
    chi_final = _chi(V, F)
    if chi_final != 1 and verbose:
        print(f"    [handle] ВНИМАНИЕ: chi={chi_final} после {n_cuts} разрезов — "
              f"диск не получен, развёртка будет со складками")

    return V, F, n_cuts, info, parents_all
...
def _arap(V, F, UV_init, face_weight, n_iter=80, tol=1e-5,
    ...
    try:
        ...
        # Страховка. Даже при верном закреплении матрица бывает почти
        # вырожденной на плохой геометрии: нулевые площади треугольников,
        # слипшиеся вершины. Регуляризация Тихонова сдвигает диагональ
        # на малую величину — решение смещается незначительно, зато оно
        # есть, вместо «Factor is exactly singular» и аварийного останова.
        from scipy.sparse import identity as _sp_eye
        _scale = float(np.abs(L_free.diagonal()).mean()) or 1.0
        L_free = (L_free + _sp_eye(L_free.shape[0], format="csc") * (1e-8 * _scale)).tocsc()
        solver = splu(L_free)
        if verbose:
            print("    [arap] лапласиан почти вырожден — включена регуляризация")
...
def _unfold_single(V, F, zone_labels, opts):
    ...
    # ── Ручки (genus > 0) ───────────────────────────────────────
    # Развёртка требует диска (chi == 1). Поверхность с ручкой разложить
    # на плоскость без наложений НЕЛЬЗЯ: ARAP растянет её как сможет,
    # и часть площади ляжет сама на себя — это и видно как «складку
    # развёртки». Раньше такая поверхность проходила молча, и врач
    # получал правдоподобную, но неверную карту.
    #
    # Типичный источник ручки на слизистой — соприкосновение перегородки
    # с латеральной стенкой: в месте контакта поверхность замыкается
    # в кольцо, и просвет вокруг становится тоннелем.
    nV_before_h, nF_before_h = len(V), len(F)
    # Края сохранённых перфораций передаём как неприкосновенные.
    # Классификатор уже решил, что это анатомия, а не артефакт:
    # разрезать их — значит вскрыть отверстие, по которому врач меряет
    # диаметры для лоскута.
    _protect = [p.get('vertex_indices') or []
                for p in (loops_info.get('preserved_perforations') or [])]
    V, F, n_handle_cuts, handle_info, handle_parents = cut_handles(
        V, F, verbose=verbose, protected_loops=_protect)
    if n_handle_cuts:
        nV_added_h = len(V) - nV_before_h
        if nV_added_h > 0:
            if handle_parents and len(handle_parents) == nV_added_h:
                # ВАЖНО: разрешаем parent'ов ПО ОДНОМУ, а не векторно.
                #
                # cut_handles() делает несколько разрезов подряд, и второй
                # режет уже по вершинам, добавленным первым. Его parent'ы
                # ссылаются на индексы, которых в исходном v_to_input нет:
                #     IndexError: index 3936 is out of bounds for size 3930
                #
                # У cut_open_inner_loops() такой беды не было — там каждый
                # разрез идёт по исходной геометрии, и векторная выборка
                # v_to_input[parents_arr] законна. Здесь же таблицу нужно
                # наращивать на каждом шаге: к моменту обработки i-го
                # parent'а все предыдущие новые вершины уже в ней есть.
                extra = np.empty(nV_added_h, dtype=np.int64)
                base = len(v_to_input)
                for i, pr in enumerate(handle_parents):
                    pr = int(pr)
                    if pr < base:
                        extra[i] = v_to_input[pr]          # исходная вершина
                    else:
                        j = pr - base                      # вершина от прошлого реза
                        extra[i] = extra[j] if 0 <= j < i else -1
                v_to_input = np.concatenate([v_to_input, extra])
            else:
                v_to_input = np.concatenate(
                    [v_to_input, np.full(nV_added_h, -1, dtype=np.int64)])
        nF_added_h = len(F) - nF_before_h
        if nF_added_h > 0:
            f_to_input = np.concatenate(
                [f_to_input, np.full(nF_added_h, -1, dtype=np.int64)])
    ...
    cycles = find_boundary_cycles(V, F)
    # Род считаем по ФАКТИЧЕСКОМУ числу граничных циклов.
    # Раньше сюда подставлялось loops_info['n_preserved'] — число
    # сохранённых перфораций, а это совсем другая величина: при
    # n_preserved = 0 и chi = -1 формула давала genus = 1.5, чего
    # не бывает. Нецелый род и был признаком ошибки.
    n_loops = len(cycles)
    genus_nom = (2 - chi - n_loops) / 2.0
    if chi != 1 and verbose:
        print(f"  [WARN] после чистки chi={chi}, границ={n_loops}, "
              f"род={genus_nom:g} — развёртка будет со складками")
    if not cycles:
    ...
    return {
        ...
        'info': {
            ...
            'n_handle_cuts': int(n_handle_cuts),
            'handle_cuts': handle_info,
            'is_disk': bool(chi == 1 and n_loops == 1),
