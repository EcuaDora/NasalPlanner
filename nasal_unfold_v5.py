...
DEFAULT_OPTS = {
    ...
    #
    # КЛИНИЧЕСКИЙ ФАКТ: перфорации слизистой бывают ТОЛЬКО на перегородке
    ...
    #
    # v4: cut_open_inner_loops резал ВСЕ петли ≥ 2мм разрезом до main-
    #     boundary. Проблема: септум-перфорацию, которую врач хочет
    #     ИЗМЕРИТЬ перед планированием лоскута, тоже разрезал.
    #
    ...
    'fill_perimeter_threshold_mm':   2.0,    # LEGACY: теперь имеет эффект
                                             # только если classify_inner_loops=False
...
def _plane_fit_centroid(P):
    ...
    n = Vt[-1]  # normal
    # Projection of centroid onto plane = c (centroid уже на плоскости best-fit).
    # Но если хотим compensate за изгиб петли, сдвинем c в среднюю "высоту" петли.
...
# -----------------------------------------------------------------------------
#
#  КЛИНИЧЕСКИЙ ФАКТ. В полости носа анатомические перфорации бывают ТОЛЬКО
#  на перегородке (septum, zone 0). Дыры на дне или на латеральной стенке —
#  всегда артефакты CT-сегментации и должны быть заполнены.
#
...
def classify_and_fill_inner_loops(V, F, zone_labels,
                                   ...
                                   verbose=False):
    """v5 для cut_open_inner_loops + smart_fill_small_holes.
    ...
    # === CUT-OPEN: крупные артефакты, стратегия из v4 =========================
...
# -----------------------------------------------------------------------------
#  CUT-OPENING (LEGACY v4 — теперь выключено по default)
# -----------------------------------------------------------------------------
#
#  ПРОБЛЕМА.
...
#  Наше положение разрезов (кратчайший путь к внешней границе) — разумный
...
def _apply_cut(V, F, path):
    ...
    Исправлено относительно v4.0:
      * ОБРАБАТЫВАЕМ ENDPOINT. На v_0 cut-edge (v_0, v_1) вместе с двумя
...
def cut_open_inner_loops(V, F, min_loop_perimeter_mm=2.0, max_loops=None,
    ...
    for inner in inners:
        ...
        # После каждого разреза топология меняется — пересчитаем границы
        # в следующей итерации. Но inners уже зафиксированы (cycles
        # посчитаны до изменений). Главная граница могла сместиться, но
        # стать короче она не может — так что следующий inner loop всё ещё
        # найдёт путь к ней. Проверка: main_verts могут быть невалидны для
        # следующей итерации, если предыдущий cut изменил нумерацию path[0].
        # На практике для реального числа (1-3) внутренних петель это работает,
        # т.к. duplicated vertices добавляются в КОНЕЦ V, а main_verts — старые
        # индексы, они остаются валидны.
...
def _compute_metrics(V, F, uv, valid, face_labels, face_areas):
    ...
    # v4.2: векторизовано + корректно пропагируется NaN. Старый for-loop
    # молча игнорировал NaN (np.nan > emax = False → emax остаётся 0 →
    # UI показывает face как «безопасный»). Теперь NaN в L2e → edge_err=NaN
    # → face позже гарантированно попадёт в bad-mask и будет risk_level=2.
...
def _unfold_single(V, F, zone_labels, opts):
    ...
    # NM-split: indexed reshuffling + duplications, нет added/removed faces
    V, F, nm_splits, orig_of = split_nonmanifold_vertices(V, F)
    v_to_input = v_to_input[orig_of]  # новые копии наследуют provenance
    ...
    # или если ISO > validity_iso_max (новое в v4 — убирает hot-spot'ы от
    # fan-fill apex и т.п. из measurement-пригодной области).
    ...
    return {
        ...
        'info': {
            ...
            'n_tiny_loops_filled':     filled_legacy,
...
def _unfold_charts(V, F, zone_labels, opts):
    ...
    # ── Необязательный polish: СЕЙЧАС EXPERIMENTAL, имеет фундаментальные
    # проблемы. Для режима measurement'а — НЕ
    # рекомендуется. Оставлен для экспериментов.
