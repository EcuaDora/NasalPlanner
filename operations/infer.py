...
PARAMS = {
    ...
    # Оставлять ВСЕ связные компоненты, а не одну крупнейшую.
    #
    # По умолчанию постобработка держит на класс одну самую большую
    # компоненту. Ноздри — две отдельные компоненты, поэтому меньшая
    # выбрасывается уже ПОСЛЕ инференса: модель её нашла, а на экран она не
    # попала. Отсюда и жалоба «размечена не та ноздря».
    #
    # Флаг --keep-all-components в CLI был давно, сюда его просто не
    # пробрасывали. Через --no-postprocess это не лечится: тот выключает
    # заодно и морфологию, то есть портит края ради одной компоненты.
...
def run(session, params):
    ...
    if p.get("keep_all_components", False):
        cmd.append("--keep-all-components")
