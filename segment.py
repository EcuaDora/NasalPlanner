...
очищенного меша. берет `mesh_clean`, пишет `inner_surface`.
...
Куда алгоритм штатно не достаёт: сильно «загнутые» краевые участки
слизистой у входа в ноздрю — там n·r противоположного знака, и
автоматически они не восстанавливаются. Это нормально — доводятся
кистью в UI-редакторе.

ЗАВИСИМОСТИ: для скорости (1-3 с на 100 тыс. фейсов) нужен embreex:
    pip install embreex
Без него работает, но в 10-30× медленнее.

КАЛИБРОВКА. Если Recall низкий (не ловим часть слизистой) — понижайте
`threshold` до -0.5 и поднимайте `alpha_hit` до 5.0. Если Precision
низкий (ловим внешний лист) — поднимайте `threshold` до 0.0 и alpha_hit
до 4.0.
"""
