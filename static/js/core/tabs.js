...
(function () {
  ...
  /* Вкладка доступна, только если выполнен её gate И все предыдущие.

     Цепочка, а не независимые проверки: каждый следующий этап спрашивает
     предыдущий. Раньше условия стояли независимо, и этап мог открыться
     через дырку в цепочке — например, метки зон в M есть, а активного
     под-меша нет, и тогда 05 доступна при запертой 04. Само по себе M в
     такое состояние не приходит, но приходит извне: восстановление из
     архива, частичный сброс, ручная правка из консоли. Достаточно
     одного случая, чтобы врач попал на этап, под которым нет данных.

     Проверка иерархическая и без побочных эффектов: gate ничего не
     чинит в M, только отвечает «нельзя». Чинить состояние — дело того,
     кто его испортил. */
  const gate = {
    segment: () => true,                   // начальный этап — всегда открыт
    data:    () => true,
    inner:   () => !!window.M.rawV,        // хотя бы сырой меш есть
    zones:   () => gate.inner() && !!window.M.V,        // + закоммичен под-меш
    unfold:  () => gate.zones() && !!window.M.zoneLabels,  // + зоны посчитаны
  };
