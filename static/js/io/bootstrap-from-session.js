...
(function () {
  ...
  async function bootstrap() {
    ...
    /* ПУСТА ЛИ СЕССИЯ — СУДИМ ПО АРТЕФАКТАМ ПАЙПЛАЙНА, А НЕ ПО ЛЮБЫМ КЛЮЧАМ.

       В сессию попадают не только данные пациента. Ещё на старте туда
       ложатся служебные ответы: infer_config_json (автопоиск путей к
       модели), ui_config_json (сохранённые цвета зон), позже
       dicom_series_json и превью. Восстанавливать в них нечего.

       Раньше проверка была «есть хоть один ключ», и на чистом запуске она
       срабатывала: конфиг успевал зарегистрироваться раньше, чем мы читали
       манифест, все шаги честно отвечали «нет данных», и врач получал
       красное «Сессия не восстановилась» на ровном месте. Пугающее и
       при этом ложное — восстанавливать было нечего с самого начала.

       Теперь смотрим только на ключи, которые кто-то умеет поднимать.
       Список совпадает с тем, что читают шаги ниже; если появится новый
       артефакт, его надо добавить и сюда — иначе сессия с ним будет
       считаться пустой. */
    const RESTORABLE = [
      'roi_ct', 'roi_mask',          // этап 01
      'mesh_raw', 'mesh_clean',      // этап 02
      'inner_surface',               // этап 03
      'zones',                       // этап 04
      'unfolded', 'paint_layer',     // этап 05
    ];
    const keys = manifest ? Object.keys(manifest) : [];
    const payload = keys.filter(k => RESTORABLE.indexOf(k) >= 0);
    if (!payload.length) {
      if (keys.length) {
        console.log(LOG + 'в сессии только служебные ключи (' +
                    keys.sort().join(', ') + ') — восстанавливать нечего');
      }
      return;
    }
    ...
    const hasCT    = await step('КТ', restoreCT);
    ...
    const hasUnfold = await step('развёртка', restoreUnfold, hasZones);
    ...
    /* Открываем тот этап, на котором врач закончил в прошлый раз.

       Правило: последний этап, чьи СОБСТВЕННЫЕ данные восстановились, —
       а не первый незаполненный. Восстановились зоны, но не развёртка →
       открываем «Зоны», а не пустую «Развёртку». Врач видит свою работу,
       а не форму для новой.

       Прыжок делается ровно здесь и только здесь. Раньше похожая логика
       жила в слушателе data:change на этапе 02 и срабатывала на ЛЮБОЙ
       приход меша — в том числе когда врач просто открыл OBJ и хотел
       посмотреть на него. Восстановление сессии — единственный случай,
       когда переключать этап за врача уместно: он сам попросил открыть
       сохранённый случай. */
    jumpToRestored({
      segment: hasCT, data: hasMesh, inner: hasInner,
      zones: hasZones, unfold: hasUnfold,
    });
  ...
  /* Переход на восстановленный этап.

     Идём сверху вниз и берём первый, который И восстановился, И реально
     открыт. Вторая проверка обязательна: gate у вкладок иерархический,
     и данные могут восстановиться частично — например, зоны легли, а
     под-меш нет. Тогда «Зоны» остаются под замком, и switchTo не просто
     промолчит, а покажет врачу предупреждение о недоступном этапе. */
  function jumpToRestored(has) {
    if (!window.Tabs || typeof window.Tabs.switchTo !== 'function') return;
    if (_userPickedTab) {
      console.log(LOG + 'этап не переключаем: врач уже выбрал вкладку сам');
      return;
    }
    const chain = ['unfold', 'zones', 'inner', 'data', 'segment'];
    for (let i = 0; i < chain.length; i++) {
      const name = chain[i];
      if (!has[name]) continue;
      const btn = document.querySelector('.tab[data-tab="' + name + '"]');
      if (btn && btn.disabled) continue;      // замок не снят — спускаемся ниже
      if (window.M && window.M.currentTab === name) return;   // уже здесь
      console.log(LOG + 'открываем восстановленный этап: ' + name);
      window.Tabs.switchTo(name);
      return;
    }
  }

  /* Восстановление асинхронное и занимает секунды. Если за это время врач
     сам ткнул во вкладку, он смотрит именно туда — выдёргивать его оттуда
     нельзя. Один клик по любой вкладке отменяет автопереход. */
  let _userPickedTab = false;
  document.addEventListener('click', (e) => {
    if (e.target && e.target.closest && e.target.closest('.tab')) _userPickedTab = true;
  }, true);
