...
(function (global) {
  ...
  /* ПОЧЕМУ СБРОС ЖИВЁТ ЗДЕСЬ, А НЕ НА ЭТАПЕ 02.

     Раньше кнопка стояла карточкой на этапе «Модель» и чистила только
     память вкладки: window.M. Серверная сессия при этом оставалась
     нетронутой. Последствий два, и оба тихие. Кнопка сохранения
     оставалась активной, потому что артефакты на сервере никуда не
     делись; а сохранённый после «сброса» архив содержал работу, которую
     врач считал стёртой. Плюс этап 01 (разметка КТ) сброс вообще не
     трогал: ROI прежнего пациента оставался на экране.

     ПОЧЕМУ ПЕРЕЗАГРУЗКА, А НЕ ТОЧЕЧНАЯ ОЧИСТКА. Тот же довод, что и при
     открытии архива: этапы держат состояние в замыканиях (редактор
     срезов на 01, editor на 03, кэши развёртки на 05), и вычистить его
     снаружи значит перечислить всё, что накопилось, и ничего не забыть.
     Перечисление устаревает молча — забытое всплывает через месяц как
     «почему тут данные прошлого пациента». Сброс сессии плюс
     перезагрузка возвращают приложение в заведомо чистое состояние, и
     список поддерживать не нужно.

     Порядок операций важен: сервер чистим ДО перезагрузки. Если сделать
     наоборот, страница поднимется, bootstrap увидит живую сессию и
     восстановит ровно то, что просили удалить. */
  async function resetSession() {
    const ask = (global.Dialog && global.Dialog.confirm)
      ? global.Dialog.confirm({
          title: 'Начать заново',
          html: 'Будут удалены <b>все</b> данные текущего сеанса: разметка КТ, ' +
                'модель, слизистая, зоны и развёртка. Несохранённое пропадёт.',
          ok: 'Сбросить',
        })
      : Promise.resolve(window.confirm(
          'Сбросить всё? Все данные текущего сеанса будут удалены.'));
    if (!(await ask)) return;

    const bt = document.getElementById('nplan-reset');
    if (bt) bt.classList.add('busy');
    try {
      const r = await fetch('/api/session', { method: 'DELETE' });
      if (!r.ok) throw new Error('HTTP ' + r.status);
    } catch (e) {
      /* Не перезагружаемся: иначе врач получит прежние данные и решит,
         что кнопка не работает, хотя на деле не ответил сервер. */
      if (bt) bt.classList.remove('busy');
      toast('Не удалось очистить сессию: ' + e.message, 'error');
      return;
    }
    reloading = true;   // глушим предупреждение beforeunload — уносить нечего
    setTimeout(() => global.location.reload(), 250);
  }
  ...
  function css() {
    ...
    st.textContent = [
      ...
      /* Разделитель перед сбросом: «сохранить» и «открыть» — обычные
         действия, «начать заново» уничтожает работу. Соседство кнопок
         одного размера делает промах дешёвым, зазор его удорожает. */
      '.nplan-sep{width:1px;height:18px;margin:0 4px;',
      '  background:var(--brd,#dfe4ec);flex:0 0 auto}',
      /* Та же оранжевая обводка, что была у кнопки на этапе 02. */
      '.nplan-btn.warn{color:var(--warn,#ff9f3c);border-color:var(--warn,#ff9f3c)}',
      '.nplan-btn.warn:hover{background:rgba(255,159,60,.10);color:var(--warn,#ff9f3c)}',
      '.nplan-btn.warn:active{background:rgba(255,159,60,.18)}',
    ].join('');
  ...
  async function refreshSaveEnabled() {
    ...
    /* Сброс гасим по тому же признаку: пустую сессию нечего сбрасывать,
       а активная кнопка при пустом сеансе выглядит как «что-то не так
       очистилось» и провоцирует лишнее нажатие. */
    const rs = document.getElementById('nplan-reset');
    if (rs) {
      rs.disabled = !has;
      rs.title = has ? 'Начать заново — удалить все данные сеанса'
                     : 'Сбрасывать пока нечего';
    }
  ...
  function mount() {
    ...
    box.innerHTML =
      ...
      '<span class="nplan-sep" aria-hidden="true"></span>' +
      '<button type="button" class="nplan-btn warn" id="nplan-reset" ' +
              'title="Начать заново — удалить все данные сеанса">' +
              ICON_RESET + '</button>' +
    ...
    box.querySelector('#nplan-reset').addEventListener('click', () => resetSession());
