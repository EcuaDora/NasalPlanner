...
(function () {
  ...
  /* Отработала ли модель на ЭТОМ КТ. Определяется наличием mask_raw в
     сессии — полноразмерного предсказания в геометрии исходного КТ.
     Оно переживает переход в режим рамки: roi_pair с mask_mode='empty'
     его не трогает, а чистится оно только при конвертации нового DICOM
     (см. _DEPENDENTS в operations/dicom_convert.py). */
  let modelMaskReady = false;
  ...
  /* Режим выбора области. 'model' — как было: инференс по всему тому и
     обрезка по bbox предсказания. 'box' — врач сам обводит рамку, модель не
     запускается, внутри рамки пусто. */
  let roiMode = 'model';        // 'model' | 'box'

  /* Переключатель режима — один выбор из двух: выбранная половина
     подсвечена, невыбранная выглядит как обычная кнопка. aria-pressed
     сообщает состояние скринридеру, для которого подсветка не существует.

     Заблокирован переключатель бывает ровно в одном случае — пока
     работает модель, см. setRoiModeEnabled(). */
  function setRoiMode(m) {
    roiMode = (m === 'box') ? 'box' : 'model';
    document.querySelectorAll('[data-roimode]').forEach((b) => {
      const on = b.dataset.roimode === roiMode;
      b.classList.toggle('active', on);
      b.setAttribute('aria-pressed', on ? 'true' : 'false');
    });
    const hint = document.getElementById('segRoiHint');
    if (hint) {
      hint.textContent = (roiMode === 'box')
        ? 'Откроется окно со срезами: обведите нужную ноздрю, затем уточните границы. ' +
          'Модель не запускается — размечать внутри рамки будете кистью.'
        : 'Область найдёт модель. Переключите, если нужна другая ноздря.';
    }
    const btn = document.getElementById('segRunBtn');
    if (btn) {
      const lbl = btn.querySelector('.seg-run-lbl');
      if (lbl) {
        lbl.textContent = (roiMode === 'box') ? 'Выбрать область'
                        : modelMaskReady ? 'Загрузить результат модели'
                        : 'Запустить сегментацию';
      }
    }
    /* Подпись в центре обещала запуск модели в обоих режимах, хотя в
       режиме куба модель не запускается вовсе. */
    const sub = document.getElementById('segEmptySub');
    if (sub) {
      sub.textContent = (roiMode === 'box')
        ? 'КТ загружен. Нажмите кнопку и обведите область на срезах.'
        : modelMaskReady
          ? 'Модель уже отработала на этом КТ — результат подгрузится сразу, без ожидания.'
          : 'КТ загружен. Нажмите кнопку — модель выделит область.';
    }
  }

  /* Пока модель работает, менять режим нельзя: инференс уже стартовал по
     одному сценарию, а переключение поменяло бы подпись кнопки и смысл
     следующего шага. Панели прячутся во время окон выбора серии и рамки
     (.seg-picking) и убираются совсем после запуска, так что это
     единственный момент, когда переключатель виден и опасен. */
  function setRoiModeEnabled(on) {
    // disabled — для клавиатуры и скринридера, класс на группе — для вида
    document.querySelectorAll('[data-roimode]').forEach((b) => { b.disabled = !on; });
    const grp = document.getElementById('segRoiMode');
    if (grp) grp.classList.toggle('is-busy', !on);
  }
  ...
  function setupStaticUI() {
    ...
    /* Проставляем режим явно: подсказки под кнопками рисуются теми же
       функциями, что и при переключении, иначе при первом открытии они
       остались бы пустыми. */
    setRoiMode(roiMode);
    ...
    stage.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.btn-run-seg')) { startSegmentation(); return; }
      const rm = e.target.closest('[data-roimode]');
      if (rm) { setRoiMode(rm.dataset.roimode); return; }
      ...
      if (shapeBtn && editor) {
        ...
        document.querySelectorAll('[data-segshape]').forEach((b) => {
          const on = (b === shapeBtn);
          b.classList.toggle('active', on);
          b.setAttribute('aria-pressed', on ? 'true' : 'false');
        });
      ...
      if (e.target.closest('[data-act="seg-region-again"]')) { restartRegion(); return; }
      /* Обработчики на [data-act="seg-download"] и [data-act="seg-obj"]
         убраны: кнопок с такими атрибутами в разметке нет ни в одной
         версии этапа. Сами downloadPair / downloadOBJ живы и вынесены в
         window.Tab0Segment — вернуть кнопку можно одной строкой. */
  ...
  function buildLeftPanel(stage) {
    ...
    left.innerHTML = [
      '<div class="card">',
        ...
        '<div class="hint-text dim" style="line-height:1.55">',
          ...
          'она выделит целевую область. Затем проверьте и поправьте разметку ',
          'прямо на срезах.',
        '</div>',
      '</div>',

      /* ВЫБОР ОБЛАСТИ. По умолчанию всё как было: модель находит область и
         обрезка идёт по bbox её предсказания.

         Второй режим нужен, когда врачу нужна не та ноздря, которую выбрала
         модель. Причина такого выбора не в самой модели: постобработка
         инференса оставляет одну самую крупную связную компоненту на класс,
         а ноздри — две отдельные компоненты, поэтому меньшая отбрасывается
         уже ПОСЛЕ инференса. Показать её нечем.

         В этом режиме модель не запускается совсем: врач обводит рамку и
         размечает внутри неё сам. Побочное следствие приятное — режим
         работает и когда путей к модели нет вовсе, то есть остаётся
         единственным способом довести случай до конца. */
      '<div class="card" id="segRoiCard">',
        '<div class="card-title">Выбор области</div>',
        '<div class="seg-tools seg-tools-2 seg-seg" id="segRoiMode" role="group">',
          '<button type="button" class="seg-mini active" data-roimode="model" aria-pressed="true" ',
                  'title="Модель сама находит область и обрезает по ней">Моделью</button>',
          '<button type="button" class="seg-mini" data-roimode="box" aria-pressed="false" ',
                  'title="Область указываете вы — кубом на срезах">Своим кубом</button>',
        '</div>',
        '<div class="hint-text dim" id="segRoiHint" style="font-size:11px;margin-top:10px;line-height:1.5;opacity:.75"></div>',
      '</div>',
      ...
      '<details class="card seg-settings" id="segModelCard">',
        ...
        /* Раз кнопка запуска теперь подгружает готовый результат, должен
           остаться способ всё-таки посчитать заново — иначе смена
           чекпойнта ни на что не влияла бы, и это было бы незаметно.

           Место выбрано по смыслу: пересчёт нужен ровно тогда, когда
           поменяли что-то выше в этой же карточке. Строка появляется
           только если считать действительно есть что переcчитывать. */
        '<div id="segRerunRow" style="display:none">',
          '<button type="button" class="seg-mini" data-act="seg-rerun" ',
                  'title="Запустить инференс заново — например, после смены файла модели" ',
                  'style="width:100%;margin-top:4px">Пересчитать сегментацию</button>',
        '</div>',
      '</details>',
    ].join('');
    ...
    if (seg) {
      seg.addEventListener('click', (e) => {
        if (e.target.closest('[data-act="seg-rerun"]')) { e.preventDefault(); rerunInfer(); return; }
  ...
  function buildEmptyState(stage) {
    ...
    empty.innerHTML = [
      ...
      '<div class="empty-icon">',
        ...
        '</svg>',
      '</div>',
      '<div class="empty-title">Разметка КТ</div>',
      ...
      '<div id="segRunMode" style="display:none">',
        ...
        '<button type="button" class="btn-open-big btn-run-seg" id="segRunBtn" style="margin-top:20px">',
          ...
          '<span class="seg-run-lbl">Запустить сегментацию</span>',
        '</button>',
      '</div>',
    ].join('');
  ...
  function buildRightPanel(stage) {
    ...
    right.innerHTML = [
      '<div class="card" id="segHowCard">',
        ...
        '<ol class="ep-steps-pre seg-steps">',
          ...
          '<li>Область находит модель. Нужна другая ноздря — переключите ',
             '<b>«Выбор области»</b> на <b>«Своим кубом»</b> и обведите её сами; ',
             'модель тогда не запускается, и разметку вы рисуете с нуля.</li>',
          '<li>Проверьте разметку на срезах: <b>Кисть</b> добавляет, ',
             '<b>Ластик</b> убирает. Пока инструмент не нажат, клик по срезу ',
             'только переставляет перекрестье.</li>',
          '<li><b>«Продолжить»</b> построит поверхность и откроет следующий этап.</li>',
        '</ol>',
      '</div>',
      '<details class="card" id="segGuideCard" style="display:none">',
        '<summary class="card-title">Как проверить и поправить</summary>',
        '<ol class="seg-steps">',
          '<li>Листайте срезы <b>колесом</b> в том окне, где курсор. Ползунок внизу листает окно, чей счётчик подсвечен; кликните по счётчику <b>АКС</b>, <b>КОР</b> или <b>САГ</b>, чтобы переключить.</li>',
          '<li><b>Двойной щелчок</b> по окну разворачивает его во весь экран, ещё один — возвращает.</li>',
          '<li><b>Кисть</b> добавляет, <b>Ластик</b> убирает; повторное нажатие на кнопку выключает инструмент. Пока ничего не выбрано, клик по срезу только переставляет перекрестье.</li>',
          '<li>Пропуск на нескольких срезах подряд? Закрасьте первый и последний, затем <b>«Протянуть между срезами»</b>.</li>',
          '<li><b>Ctrl+Z</b> отменяет последнее действие, <b>Ctrl+Y</b> возвращает.</li>',
        '</ol>',
      '</details>',
      '<div class="card" id="segToolsCard" style="display:none">',
        ...
        '<div class="seg-adv-block" id="segBrushBlock">',
          ...
          '</div>',
          '<div class="seg-tools seg-tools-2" style="margin-top:10px" role="group">',
            '<button type="button" class="seg-mini active" data-segshape="ball" aria-pressed="true" title="Кисть-шар: захватывает соседние срезы">\u25CF Шар</button>',
            '<button type="button" class="seg-mini" data-segshape="flat" aria-pressed="false" title="Плоская кисть: только видимый срез">\u25CB Плоско</button>',
          '</div>',
        ...
        '<div class="seg-actions">',
          '<button type="button" class="seg-mini" data-act="seg-clean" title="Удалить отдельные пятна вне основной области и закрыть внутренние пустоты">' + _miniIco('clean') + 'Убрать</button>',
          '<button type="button" class="seg-mini" data-act="seg-patch" title="Закрыть пустоты внутри области — полости и мелкие проколы">' + _miniIco('fill') + 'Заполнить</button>',
        '</div>',
        '<div class="seg-actions" style="margin-top:10px">',
          '<button type="button" class="seg-mini" data-act="seg-close" title="Затянуть сквозные разрывы и вмятины в стенке. Чем больше размер кисти, тем крупнее разрыв затянется">' + _miniIco('close') + 'Затянуть</button>',
          '<button type="button" class="seg-mini" data-act="seg-smooth" title="Убрать ступенчатость на границе области">' + _miniIco('smooth') + 'Сгладить</button>',
        '</div>',
        '<button type="button" class="seg-mini" data-act="seg-interp" title="Закрасьте область на первом и последнем срезе пропуска, затем нажмите — промежуточные заполнятся сами" style="width:100%;margin-top:10px">' + _miniIco('interp') + 'Протянуть между срезами</button>',
        ...
        '</div>',
        '<button type="button" class="seg-mini seg-danger" data-act="seg-reset" title="Отменить все правки и вернуть разметку, которую предложила модель" style="width:100%;margin-top:10px">' + _miniIco('reset') + 'Вернуть как было</button>',

        /* Выход из тупика «модель выбрала не ту ноздрю».

           Переключатель режима живёт в левой панели, а она после запуска
           убирается совсем — вернуться к выбору области было нечем, кроме
           кнопки «Начать заново» в шапке, которая стирает весь сеанс
           вместе с загруженным DICOM.

           Кнопка стоит в группе с другими откатами, а не под «Продолжить»:
           там её задевали бы промахом мимо основного действия. */
        '<div class="seg-tgroup-lbl">Область</div>',
        '<button type="button" class="seg-mini seg-danger" data-act="seg-region-again" title="Вернуться к выбору области. КТ останется загруженным, размечать начнёте заново" style="width:100%">' + _miniIco('region') + 'Выбрать область заново</button>',
        ...
        '<label class="seg-check" style="margin-top:12px">',
          '<input type="checkbox" id="segCrosshair" checked> показывать перекрестье',
        '</label>',
      '</div>',
    ...
    /* Слушателя на [data-wl] (пресеты окна КТ) и на #segContour больше
       нет: ни кнопок, ни галочки с такими id в разметке этапа не было. */
    right.addEventListener('change', (e) => {
      if (!editor) return;
      if (e.target.id === 'segCrosshair') editor.setCrosshair(e.target.checked);
    });
  }

  // ─── Переключение режимов центра: загрузка ↔ запуск ──────────────────
  /* Спрашиваем сервер, а не помним сами: так флаг верен и после
     открытия архива, и после перезагрузки страницы, и после «Начать
     заново» — во всех трёх случаях состояние приходит из манифеста,
     а не из памяти вкладки. */
  async function refreshModelMaskFlag() {
    try {
      const m = await fetch('/api/session').then((r) => r.json());
      modelMaskReady = !!(m && m.mask_raw);
    } catch (e) {
      modelMaskReady = false;
    }
    setRoiMode(roiMode);   // подписи кнопки и центра зависят от флага
    const rerun = document.getElementById('segRerunRow');
    if (rerun) rerun.style.display = modelMaskReady ? '' : 'none';
  }
  ...
  window.addEventListener('ct:change', () => {
    ctReady = true; refreshRunAvailability(); refreshModelMaskFlag();
  });
  ...
  async function useSeries(uid) {
    ...
    try {
      ...
      if (roiMode === 'box') {
        /* Своя рамка всегда даёт пустую маску, инференс не запускается —
           значит и пути к модели не нужны. Не держим врача из-за настроек,
           которые не будут использованы. */
        _spinHide();
        await openBoxPicker();
      } else if (modelCfgReady()) {
        /* Режим решает, что запускать: привычный инференс или окно с
           кубом. Раньше здесь был безусловный runInfer, и выбрать другую
           ноздрю было негде — модель стартовала сама. */
        if (roiMode === 'box') { _spinHide(); await openBoxPicker(); }
        else await runInfer();       // сам держит спиннер и ловит ошибки
      } else {
  ...
  // ═══════════════════════════════════════════════════════════
  // ═══════════════════════════════════════════════════════════
  // Выбор области кубом (режим «Своим кубом»)
  // ═══════════════════════════════════════════════════════════

  /* ПОЧЕМУ КУБ РИСУЕТСЯ НА ПРЕВЬЮ ИЗ ct_raw, А НЕ НА ПРЕВЬЮ СЕРИИ.

     Превью серии (dicom_preview) уже загружено к этому моменту и напрашивается
     само. Но брать его нельзя по двум причинам, и каждой достаточно.

     Первая: оно строится из DICOM через DICOMOrient(img, "LPS"), а конвертация
     в ct_raw ориентацию не трогает. Оси совпадают не на всех сериях, и куб
     поехал бы при переносе — молча, на части случаев.

     Вторая: ct_raw приходит не только из DICOM. Через ct-loader.js врач
     открывает готовый .nrrd, и превью серии тогда нет вовсе.

     Поэтому есть операция ct_preview: прореженная копия САМОГО ct_raw. Её
     индексы — это индексы ct_raw, поделённые на целый шаг, поэтому доли по
     осям переносятся обратно без всякой геометрии. */

  let _boxPickerState = null;

  async function openBoxPicker() {
    const vp = document.querySelector('.stage[data-stage="segment"] .viewport');
    const stage = document.querySelector('.stage[data-stage="segment"]');
    if (!vp || !stage) return;
    if (document.getElementById('segBoxPicker')) return;

    let meta, vol;
    try {
      _spin('Подготовка обзора…');
      await sse('/api/ct_preview/stream', {}, _spinText);
      meta = await fetch('/api/session/ct_preview_json', { cache: 'no-store' })
        .then((r) => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); });
      const buf = await fetch('/api/session/' + encodeURIComponent(meta.volume), { cache: 'no-store' })
        .then((r) => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.arrayBuffer(); });
      vol = new Uint8Array(buf);
      _spinHide();
    } catch (err) {
      _spinHide();
      _toast('Не удалось построить обзор КТ: ' + err.message, 'err', 7000);
      return;
    }
    ...
    const XY = X * Y;
    if (vol.length < XY * Z) {
      _toast('Обзор КТ повреждён: размер тома не совпал с dims', 'err', 7000);
      return;
    }

    /* ДВЕ ФАЗЫ, И ПЕРЕЙТИ ИЗ ВТОРОЙ В ПЕРВУЮ МОЖНО ТОЛЬКО СБРОСОМ.

       Пока box === null идёт обводка: тянем в любом окне и получаем рамку.
       Как только она есть, обводка выключается, остаётся правка границ. Это
       не придирка к порядку действий, а защита от потери работы: рисование и
       правка живут на одном холсте и на одной кнопке мыши, и если оставить
       их вперемешку, то промах мимо края в фазе правки стирал бы всю
       выверенную рамку одним движением. Отменить это было бы нечем.

       Третья ось при обводке остаётся полной: одно окно задаёт только две
       свои оси, и делать вид, что третья тоже выбрана, нельзя. Врач сузит её
       в соседнем окне, потянув за край. */
    let box = null;
    const st = { z: Z >> 1, cy: Y >> 1, cx: X >> 1 };
    ...
    ov.innerHTML = [
      '<div class="seg-picker-box seg-picker-wide">',
        '<button type="button" class="seg-picker-close" data-box="cancel" title="Закрыть" aria-label="Закрыть">&times;</button>',
        '<div class="seg-picker-title">Выбор области</div>',
        '<div class="seg-picker-hint" id="segBoxHint"></div>',
        '<div class="seg-vv-wrap" style="margin-top:6px">',
          '<div class="seg-vv seg-vv-c">',
            '<div class="seg-vv-cell c-ax"><span class="seg-vv-lbl">Аксиальный</span><canvas data-bp="ax"></canvas></div>',
            '<div class="seg-vv-cell c-sag"><span class="seg-vv-lbl">Сагиттальный</span><canvas data-bp="sag"></canvas></div>',
            '<div class="seg-vv-cell c-cor"><span class="seg-vv-lbl">Корональный</span><canvas data-bp="cor"></canvas></div>',
          '</div>',
        '</div>',
        '<div class="seg-picker-actions" style="gap:10px">',
          '<span class="hint-text dim" id="segBoxSize" style="font-size:12px;opacity:.8"></span>',
          '<button type="button" class="seg-mini" data-box="reset" ',
                  'title="Убрать рамку и обвести область заново">Сбросить рамку</button>',
          '<button type="button" class="btn-open-big" data-box="apply" id="segBoxApply" ',
                  'style="min-width:220px;justify-content:center">Применить</button>',
        '</div>',
      '</div>',
    ].join('');
    vp.appendChild(ov);
    stage.classList.add('seg-picking');

    const planes = {
      ax:  { w: X, h: Y, sp: [sp[0], sp[1]], cols: 'x', rows: 'y' },
      cor: { w: X, h: Z, sp: [sp[0], sp[2]], cols: 'x', rows: 'z' },
      sag: { w: Y, h: Z, sp: [sp[1], sp[2]], cols: 'y', rows: 'z' },
    };
    ...
    /* Рамка хранится в долях объёма, а рисуется в долях ЭКРАНА. По оси z эти
       два не совпадают: на корональном и сагиттальном голова смотрит вверх,
       поэтому строка r показывает воксель Z-1-r. Переворот живёт ровно в этих
       двух функциях, всё остальное работает с экранными долями и о нём не
       знает. */
    const clamp01 = (v) => Math.max(0, Math.min(1, v));
    const MINF = 0.02;      // минимальная сторона рамки в долях

    function readRange(pl, which) {
      const P = planes[pl];
      if (which === 'cols') return [box[P.cols + '0'], box[P.cols + '1']];
      if (P.rows === 'z')   return [1 - box.z1, 1 - box.z0];
      return [box[P.rows + '0'], box[P.rows + '1']];
    }
    function writeRange(pl, which, f0, f1) {
      const P = planes[pl];
      if (f1 < f0) { const t = f0; f0 = f1; f1 = t; }
      f0 = clamp01(f0); f1 = clamp01(f1);
      if (f1 - f0 < MINF) {
        if (f0 + MINF <= 1) f1 = f0 + MINF; else f0 = f1 - MINF;
      }
      if (which === 'cols') { box[P.cols + '0'] = f0; box[P.cols + '1'] = f1; return; }
      if (P.rows === 'z')   { box.z0 = 1 - f1; box.z1 = 1 - f0; return; }
      box[P.rows + '0'] = f0; box[P.rows + '1'] = f1;
    }

    function fitOf(cv, pl) {
      const P = planes[pl];
      const pw = P.w * P.sp[0], ph = P.h * P.sp[1];
      const sc = Math.min(cv.width / pw, cv.height / ph);
      const dw = pw * sc, dh = ph * sc;
      return { ox: (cv.width - dw) / 2, oy: (cv.height - dh) / 2, dw, dh };
    }

    function rectOf(cv, pl) {
      const f = fitOf(cv, pl);
      const [c0, c1] = readRange(pl, 'cols');
      const [r0, r1] = readRange(pl, 'rows');
      return {
        x0: f.ox + c0 * f.dw, x1: f.ox + c1 * f.dw,
        y0: f.oy + r0 * f.dh, y1: f.oy + r1 * f.dh, f,
      };
    }

    function drawPlane(cv, pl) {
      const P = planes[pl];
      const img = new ImageData(P.w, P.h), px = img.data;
      for (let r = 0; r < P.h; r++) {
        for (let c = 0; c < P.w; c++) {
          const g = sample(pl, c, r), o = (r * P.w + c) * 4;
          px[o] = px[o + 1] = px[o + 2] = g; px[o + 3] = 255;
      ...
      const dpr = window.devicePixelRatio || 1;
      const rc = cv.parentElement.getBoundingClientRect();
      const W = Math.max(2, Math.floor(rc.width * dpr));
      const H = Math.max(2, Math.floor(rc.height * dpr));
      ...
      const f = fitOf(cv, pl);
      cx.imageSmoothingEnabled = true;
      cx.drawImage(tmp, f.ox, f.oy, f.dw, f.dh);

      if (!box) return;                       // фаза обводки — рисовать нечего

      const R = rectOf(cv, pl);
      // затемняем всё вне рамки — сразу видно, что уйдёт в работу
      cx.fillStyle = 'rgba(6,12,22,.55)';
      cx.beginPath();
      cx.rect(f.ox, f.oy, f.dw, f.dh);
      cx.rect(R.x0, R.y0, R.x1 - R.x0, R.y1 - R.y0);
      cx.fill('evenodd');

      cx.strokeStyle = 'rgba(120,220,255,.95)';
      cx.lineWidth = Math.max(1.5, 1.6 * dpr);
      cx.strokeRect(R.x0, R.y0, R.x1 - R.x0, R.y1 - R.y0);

      // маркеры на углах и серединах сторон — показывают, за что тянуть
      const hs = 3.5 * dpr;
      cx.fillStyle = 'rgba(120,220,255,.95)';
      const mxs = [R.x0, (R.x0 + R.x1) / 2, R.x1];
      const mys = [R.y0, (R.y0 + R.y1) / 2, R.y1];
      for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
          if (i === 1 && j === 1) continue;
          cx.fillRect(mxs[i] - hs, mys[j] - hs, hs * 2, hs * 2);
        }
      }
    }

    function syncChrome() {
      const hint = ov.querySelector('#segBoxHint');
      if (hint) {
        hint.innerHTML = box
          ? ('<b>Рамка задана.</b> Потяните за край или угол, чтобы поправить границу, ' +
             'за середину — чтобы сдвинуть целиком. Каждое окно задаёт свои две оси. ' +
             'Чтобы обвести заново — «Сбросить рамку». <b>Колесо</b> листает срезы.')
          : ('<b>Обведите область</b> протяжкой в любом из окон — оно задаст две оси. ' +
             'Третья останется полной, сузить её можно в соседнем окне, потянув за край. ' +
             '<b>Колесо</b> листает срезы.');
      }
      const apply = ov.querySelector('#segBoxApply');
      if (apply) {
        apply.classList.toggle('is-disabled', !box);
        apply.title = box ? '' : 'Сначала обведите область';
      }
      const el = ov.querySelector('#segBoxSize');
      if (el) {
        if (!box) { el.textContent = 'Область не задана'; return; }
        const full = meta.full_dims || [X, Y, Z];
        const nx = Math.round((box.x1 - box.x0) * full[0]);
        const ny = Math.round((box.y1 - box.y0) * full[1]);
        const nz = Math.round((box.z1 - box.z0) * full[2]);
        el.textContent = 'Область: ' + nx + '×' + ny + '×' + nz + ' вокселей';
      }
    }

    function drawAll() {
      ov.querySelectorAll('canvas[data-bp]').forEach((cv) => drawPlane(cv, cv.dataset.bp));
      syncChrome();
    }

    function eventToCanvas(cv, e) {
      const rc = cv.getBoundingClientRect();
      const dpr = window.devicePixelRatio || 1;
      return { mx: (e.clientX - rc.left) * dpr, my: (e.clientY - rc.top) * dpr };
    }

    function hitTest(cv, pl, e) {
      if (!box) return null;
      const { mx, my } = eventToCanvas(cv, e);
      const R = rectOf(cv, pl);
      const dpr = window.devicePixelRatio || 1;
      const H = 9 * dpr;
      const nearX0 = Math.abs(mx - R.x0) <= H, nearX1 = Math.abs(mx - R.x1) <= H;
      const nearY0 = Math.abs(my - R.y0) <= H, nearY1 = Math.abs(my - R.y1) <= H;
      if (mx < R.x0 - H || mx > R.x1 + H || my < R.y0 - H || my > R.y1 + H) return null;
      const ex = nearX0 ? -1 : (nearX1 ? 1 : 0);
      const ey = nearY0 ? -1 : (nearY1 ? 1 : 0);
      if (ex || ey) return { mode: 'resize', ex, ey };
      if (mx > R.x0 && mx < R.x1 && my > R.y0 && my < R.y1) return { mode: 'move', ex: 0, ey: 0 };
      return null;
    }

    function cursorFor(h) {
      if (!box) return 'crosshair';           // фаза обводки
      if (!h) return 'default';
    ...
    let drag = null;
    ov.querySelectorAll('canvas[data-bp]').forEach((cv) => {
      const pl = cv.dataset.bp;
      cv.style.cursor = 'crosshair';
      cv.addEventListener('mousedown', (e) => {
        if (e.button !== 0) return;
        const f = fitOf(cv, pl);
        const { mx, my } = eventToCanvas(cv, e);
        const startC = (mx - f.ox) / f.dw, startR = (my - f.oy) / f.dh;

        if (!box) {                            // ── фаза 1: обводим ──
          e.preventDefault();
          box = { x0: 0, x1: 1, y0: 0, y1: 1, z0: 0, z1: 1 };
          drag = { cv, pl, mode: 'create', startC, startR };
          writeRange(pl, 'cols', startC, startC);
          writeRange(pl, 'rows', startR, startR);
          drawAll();
          return;
        }
        // ── фаза 2: только правка, мимо рамки — ничего ──
        const h = hitTest(cv, pl, e);
        if (!h) return;
        e.preventDefault();
        drag = {
          cv, pl, mode: h.mode, ex: h.ex, ey: h.ey, startC, startR,
          origC: readRange(pl, 'cols').slice(),
          origR: readRange(pl, 'rows').slice(),
        };
      });
      cv.addEventListener('mousemove', (e) => {
        if (drag) return;
        cv.style.cursor = cursorFor(hitTest(cv, pl, e));
      });
      cv.addEventListener('wheel', (e) => {
        e.preventDefault();
        const d = e.deltaY > 0 ? 1 : -1;
        if (pl === 'ax') st.z = Math.max(0, Math.min(Z - 1, st.z + d));
        else if (pl === 'cor') st.cy = Math.max(0, Math.min(Y - 1, st.cy + d));
        else st.cx = Math.max(0, Math.min(X - 1, st.cx + d));
        drawAll();
      }, { passive: false });
    });
    ...
    function applyDrag(e) {
      if (!drag) return;
      const f = fitOf(drag.cv, drag.pl);
      const { mx, my } = eventToCanvas(drag.cv, e);
      const fc = (mx - f.ox) / f.dw, fr = (my - f.oy) / f.dh;

      if (drag.mode === 'create') {
        writeRange(drag.pl, 'cols', drag.startC, fc);
        writeRange(drag.pl, 'rows', drag.startR, fr);
      } else if (drag.mode === 'move') {
        // сдвиг целиком: ширина сохраняется, поэтому упираемся в край, а не мнёмся
        const dc = fc - drag.startC, dr = fr - drag.startR;
        const wc = drag.origC[1] - drag.origC[0], wr = drag.origR[1] - drag.origR[0];
        const c0 = Math.max(0, Math.min(1 - wc, drag.origC[0] + dc));
        const r0 = Math.max(0, Math.min(1 - wr, drag.origR[0] + dr));
        writeRange(drag.pl, 'cols', c0, c0 + wc);
        writeRange(drag.pl, 'rows', r0, r0 + wr);
      } else {
        let [c0, c1] = drag.origC, [r0, r1] = drag.origR;
        if (drag.ex === -1) c0 = fc; else if (drag.ex === 1) c1 = fc;
        if (drag.ey === -1) r0 = fr; else if (drag.ey === 1) r1 = fr;
        if (drag.ex) writeRange(drag.pl, 'cols', c0, c1);
        if (drag.ey) writeRange(drag.pl, 'rows', r0, r1);
      }
      drawAll();
    }
    const onMove = (e) => { if (drag) applyDrag(e); };
    const onUp = (e) => {
      if (!drag) return;
      applyDrag(e);
      ...
      drag = null;
      if (wasCreate) {
        // курсор сразу переключаем на «правку», иначе он останется крестом
        ov.querySelectorAll('canvas[data-bp]').forEach((c) => { c.style.cursor = 'default'; });
      }
    };
    ...
    function close() {
      ...
      ov.remove();
      stage.classList.remove('seg-picking');
      _boxPickerState = null;
    }
    _boxPickerState = { close };

    ov.addEventListener('click', (e) => {
      const b = e.target.closest('[data-box]');
      if (!b) return;
      const act = b.dataset.box;
      if (act === 'cancel') { close(); return; }
      if (act === 'reset') {
        box = null;                            // всё сначала: обводка, затем правка
        drag = null;
        ov.querySelectorAll('canvas[data-bp]').forEach((c) => { c.style.cursor = 'crosshair'; });
        drawAll();
        return;
      }
      if (act === 'apply') {
        if (!box) { _toast('Сначала обведите область в любом из окон', 'warn', 3500); return; }
        const chosen = Object.assign({}, box);
        close();
        runWithBox(chosen);
      }
    });
  ...
  /* Общий хвост обоих режимов: забрать пару ROI из сессии и поставить
     редактор. Раньше этот кусок жил только внутри runInfer; с появлением
     второго пути его пришлось бы копировать, а две копии загрузки редактора
     разъезжаются ровно так же, как разъехались когда-то цвета зон. */
  async function installRoiFromSession() {
    const okBuf = (k) => (r) => {
  ...
  /* Возврат к выбору области, не теряя загруженный КТ.

     Нужен ровно для одного случая: модель выделила не ту половину носа.
     Раньше выход был только через «Начать заново» в шапке — а она сносит
     весь сеанс, и папку DICOM приходилось выбирать и конвертировать
     заново, минуты на пустом месте.

     ct_raw в сессии остаётся, поэтому шаг назад стоит доли секунды:
     закрываем редактор, возвращаем левую панель с переключателем и
     центральную кнопку запуска. Следующий запуск перезапишет roi_ct и
     roi_mask, поэтому чистить их отдельно не нужно. */
  function restartRegion() {
    if (!editor) return;
    segConfirm('Разметка этой области будет потеряна. КТ останется загруженным — ' +
               'вы сразу сможете выбрать область заново.', {
      title: 'Выбрать область заново?',
      okLabel: 'Выбрать заново', cancelLabel: 'Отмена', variant: 'warn',
    }).then((ok) => {
      if (!ok || !editor) return;
      editor.dispose();
      editor = null;
      showEditorChrome(false);      // вернуть левую панель и центральный экран
      refreshRunAvailability();     // КТ на месте → сразу режим запуска
      setRoiMode(roiMode);          // подписи и подсказки под текущий режим
      _toast('Выберите область: <strong>«Моделью»</strong> или <strong>«Своим кубом»</strong> ' +
             'в левой панели, затем запустите.', 'info', 6000, { html: true });
    });
  }

  /* Точка входа кнопки запуска и автозапуска после конвертации серии.
     Режим решает, что произойдёт: привычный инференс по всему тому или
     сначала окно с кубом. */
  function startSegmentation() {
    if (!ctReady) { _toast('Сначала загрузите КТ (папку DICOM)', 'warn', 4000); return; }
    if (roiMode === 'box') { openBoxPicker(); return; }
    if (modelMaskReady) { reuseModelResult(); return; }
    runInfer();
  }

  /* Повторный вход в режим «Моделью» без повторного инференса.

     Врач выбрал модель, посмотрел, ушёл рисовать рамкой, передумал — и
     раньше ждал полный прогон заново, хотя ничего не изменилось.

     Ждать нечего: mask_raw — предсказание в геометрии исходного КТ —
     лежит в сессии нетронутым. Режим рамки вызывает roi_pair с
     mask_mode='empty', а эта ветка mask_raw даже не читает. Значит
     достаточно повторить ровно тот же вызов, что делает runInfer после
     инференса: roi_pair без box и с mask_mode по умолчанию сам найдёт
     bbox по маске и нарежет пару заново. Это обрезка массива — секунды
     против минут.

     Восстанавливается предсказание модели в исходном виде: правки
     первого захода жили в roi_mask, а его режим рамки перезаписал.
     Врач это уже подтвердил, когда нажимал «Выбрать область заново». */
  /* Явный пересчёт. Спрашиваем подтверждение: инференс идёт минуты, и
     запустить его случайным кликом обиднее, чем лишний раз ответить. */
  function rerunInfer() {
    segConfirm('Модель посчитает разметку заново. Это занимает несколько минут. ' +
               'Текущий результат будет заменён.', {
      title: 'Пересчитать сегментацию?',
      okLabel: 'Пересчитать', cancelLabel: 'Отмена', variant: 'warn',
    }).then((ok) => { if (ok) runInfer(); });
  }

  async function reuseModelResult() {
    const runBtn = document.getElementById('segRunBtn');
    if (runBtn) runBtn.disabled = true;
    setRoiModeEnabled(false);
    try {
      _spin('Загрузка результата модели…');
      await postJSON('/api/roi_pair', {});
      await installRoiFromSession();
      _spinHide();
      _toast('<strong>Готово.</strong> Загружен результат модели — тот же, что и в прошлый раз. ' +
             'Инференс не запускался.', 'ok', 6000, { html: true });
    } catch (err) {
      _spinHide();
      console.error('[tab0-segment] повторная загрузка результата:', err);
      /* Единственная ожидаемая причина — mask_raw всё-таки пропал
         (открыли другой архив, сбросили сессию). Тогда честнее не
         ругаться, а просто посчитать. */
      modelMaskReady = false;
      setRoiMode(roiMode);
      _toast('Сохранённого результата не оказалось — запускаю модель.', 'warn', 5000);
      runInfer();
    } finally {
      if (runBtn) runBtn.disabled = false;
      setRoiModeEnabled(true);
    }
  }

  /* Ветка «Своим кубом»: обрезаем КТ по рамке и отдаём врачу пустую маску.

     Инференс здесь не запускается вовсе — ни модель, ни её настройки не
     участвуют. Поэтому режим работает и тогда, когда пути к модели не
     заполнены: это единственный путь довести случай до конца, если модель
     недоступна. */
  async function runWithBox(box) {
    const runBtn = document.getElementById('segRunBtn');
    if (runBtn) runBtn.disabled = true;
    setRoiModeEnabled(false);
    try {
      _spin('Обрезка по выбранной области…');
      await postJSON('/api/roi_pair', { box: box, mask_mode: 'empty' });
      await installRoiFromSession();
      _spinHide();
      _toast('<strong>Область готова.</strong> Маска пустая — размечайте кистью. ' +
             'Инструмент включается кнопкой «Кисть», повторное нажатие выключает.',
             'ok', 8000, { html: true });
    } catch (err) {
      _spinHide();
      console.error('[tab0-segment] рамка:', err);
      _toast('<strong>Ошибка:</strong> ' + err.message, 'err', 9000, { html: true });
    } finally {
      if (runBtn) runBtn.disabled = false;
      setRoiModeEnabled(true);
    }
  ...
  async function runInfer() {
    ...
    try {
      ...
      await postJSON('/api/roi_pair', {});
      /* mask_raw теперь в сессии — повторный вход в режим «Моделью»
         обойдётся без инференса. */
      modelMaskReady = true;

      // 3) Грузим ROI КТ + маску и ставим редактор (общий код с режимом куба)
      await installRoiFromSession();
...
function keepLargest(m, Z,Y,X){
  ...
  /* НИ ОДНОЙ КОМПОНЕНТЫ — ВОЗВРАЩАЕМ ПУСТО, А НЕ ВСЁ.

     bestId остаётся нулём, а lab целиком заполнен нулями, поэтому проверка
     lab[i]===bestId ниже была истинной для КАЖДОГО вокселя: «оставить самую
     крупную компоненту» на пустой маске размечало весь объём. Пока маска
     всегда приходила от модели и пустой не бывала, ветка не срабатывала; с
     ручным выбором области маска стартует нулевой, и первое же нажатие
     «Убрать» красило всю область целиком. */
  if(!bestId) return new Uint8Array(n);
...
return {airMask,sqDistToAir,keepLargest,fillHoles3D,dilate,cleanup,shaveToLining,eraserBall,brushLining,paintBall,airThreshold,erode,close,sliceMorphAxis,fill2DHolesAxial};
  ...
  const SliceEditor = (function () {
    ...
    function install(ctVol, maskVol) {
      ...
      const s = {
        ...
        tool: null,          // инструмент не выбран, пока врач не нажмёт кнопку
        crosshair: true,     // показывать перекрестье на срезах
        radius: 2, alpha: 0.55,
        ...
        /* Полей от скрибл-достройки, лассо и пипетки здесь больше нет.
           Инструментов под них в панели не было, а sFg/sBg выделяли по
           Uint8Array на весь объём — на реальном ROI это 51 МБ, которые
           аллоцировались при каждом открытии этапа и проверялись в цикле
           отрисовки на каждый пиксель каждого из трёх окон. */
      ...
      function closeFill(){ pushHistory();
        ...
        render(); if(s.r3d) s.r3d.markDirty();
        _toast(added ? 'Разрывы затянуты'
                     : 'Затягивать нечего — область сплошная', added?'ok':'info', 3000); }
      function patchHoles(){ pushHistory();
        ...
        render(); if(s.r3d) s.r3d.markDirty();
        if (added || removed || air) _toast('Пустоты внутри области закрыты', 'ok', 3000);
        else _toast('Закрытых пустот нет. Если разрыв сквозной — нажмите «Затянуть» ' +
                    '(для крупного увеличьте размер кисти).', 'info', 5000); }
      function cleanupNow(){ pushHistory();
        ...
        s.mask.set(m); render(); if(s.r3d) s.r3d.markDirty();
        _toast((removed||added) ? 'Лишние пятна убраны, пустоты закрыты'
                                : 'Здесь уже чисто — убирать нечего', (removed||added)?'ok':'info', 3000); }
      (function initAnnot(){
      ...
      function renderPane(pane) {
        ...
        for (let pr = 0; pr < h2d; pr++) {
          for (let pc = 0; pc < w2d; pc++) {
            ...
            let R, G, B;
            if (m[vi]) { R = Math.round(g * (1 - a) + 255 * a); G = Math.round(g * (1 - a)); B = Math.round(g * (1 - a)); }
            else { R = g; G = g; B = g; }
            const o = (pr * w2d + pc) * 4;
        ...
        /* Перекрестье текущей позиции. Приглушено намеренно: прежняя пара
           (тёмная окантовка 3.5 px под яркой линией 1.5 px) перекрывала
           границу маски именно там, куда врач смотрит, — линия проходит
           ровно через точку интереса. Теперь тоньше и полупрозрачно, а
           совсем убрать можно галочкой в «Отображении». */
        if (s.crosshair) {
          ...
          drawCross('rgba(0,0,0,.22)', 2.5);          // тёмная окантовка
          drawCross('rgba(120,220,255,.42)', 1.0);    // линия
        }
      }
      ...
      /* ─── ИСТОРИЯ ОТМЕНЫ: ДЕЛЬТЫ, А НЕ СНИМКИ ──────────────────────
         Было: каждый шаг клал в стек полную копию маски. На реальном ROI
         (порядка 230×409×285) это 25 МБ за шаг. Глубина 30 плюс столько
         же в «Повторить» — три четверти гигабайта в стеке отмены. Чем
         аккуратнее работал врач, тем вернее падала вкладка.

         Стало: шаг хранит только изменённые воксели — их индексы и
         значение ДО правки. Мазок кистью весит десятки килобайт вместо
         25 МБ, поэтому глубину можно поднять до 60 и остаться в единицах
         мегабайт.

         Схема двухфазная: pushHistory() запоминает состояние ДО правки
         (одна полная копия, живёт до конца операции), commitHistory()
         превращает её в дельту. Коммит ленивый — его делает следующий
         pushHistory, а также undo, redo и конец мазка. Так не пришлось
         дописывать вызов в конец каждой операции: забыли бы ровно в той,
         которой пользуются реже всего.

         Ещё одно следствие: undo больше не подменяет s.mask новым
         массивом, а правит существующий по индексам. Ссылка, которую
         держит 3D-вью, остаётся живой. */
      const HIST_MAX = 60;
      let _histBefore = null;      // полная копия «до» текущей операции

      function _diffFrom(before) {
        const cur = s.mask, n = cur.length;
        let cnt = 0;
        for (let i = 0; i < n; i++) if (before[i] !== cur[i]) cnt++;
        if (!cnt) return null;
        const idx = new Int32Array(cnt), val = new Uint8Array(cnt);
        let k = 0;
        for (let i = 0; i < n; i++) {
          if (before[i] !== cur[i]) { idx[k] = i; val[k] = before[i]; k++; }
        }
        return { idx: idx, val: val };
      }

      function commitHistory() {
        if (!_histBefore) return;
        const d = _diffFrom(_histBefore);
        _histBefore = null;
        /* Операция ничего не изменила (нажали «Сгладить» на уже гладкой
           маске) — в стек не пишем: иначе Ctrl+Z срабатывал бы вхолостую
      ...
      function pushHistory() {
        commitHistory();
        _histBefore = Uint8Array.from(s.mask);
      }

      // применить дельту к маске и вернуть обратную — для второго стека
      function _applyDelta(d) {
        const cur = s.mask, idx = d.idx, val = d.val, n = idx.length;
      ...
      function runInterp() {
        ...
        if (!plan.length) {
          _toast('Сначала закрасьте область кистью на двух срезах одного окна — ' +
                 'на первом и последнем срезе пропуска. Затем нажмите «Протянуть между срезами».', 'info', 6000);
          return;
        }
        pushHistory();
        let filled = 0, gaps = 0;
        ...
        const air = stripAir();                          // авто-очистка воздуха в конце
        render(); if (s.r3d) s.r3d.markDirty();
        _toast(gaps ? ('Пропуск заполнен: промежутков между размеченными срезами — ' + gaps)
                    : 'Между размеченными срезами пропусков нет — заполнять нечего', gaps ? 'ok' : 'info', 3500);
      }

      // ── Сглаживание маски: 3D-голосование 3×3×3 (убирает «лесенку») ──
      function smoothMask() {
        let nz = 0; for (let i = 0; i < s.mask.length; i++) if (s.mask[i]) { nz = 1; break; }
        if (!nz) { _toast('Разметка пустая — сглаживать нечего', 'info', 2500); return; }
        pushHistory();
        ...
        /* set, а не присваивание ссылки. s.mask роздан наружу: 3D-вью
           держит его через getMask, история — через свои копии. Подмена
           объекта оставляла их работать со старым массивом, и после
           «Сгладить» 3D-окно показывало домазковую форму, а отмена
           возвращала не то, что было на экране. */
        s.mask.set(out);
        render(); if (s.r3d) s.r3d.markDirty();
        _toast('Край сглажен', 'ok', 2200);
      }
      ...
      function onDown(pane, e) {
        ...
        setCrosshairVoxel(v);
        /* Инструмент не выбран — только навигация. История не трогается:
           перенос перекрестья не изменение маски, и засорять им отмену
           нельзя, иначе Ctrl+Z начнёт «откатывать» простые клики. */
        if (!s.tool) { render(); return; }
        pushHistory();
      ...
      function onUp() {
        ...
        commitHistory();   // мазок закончен — он и есть один шаг отмены
        if (s.r3d) s.r3d.markDirty();
      }
      ...
      /* ГОРЯЧИЕ КЛАВИШИ ЭТОГО ЭТАПА.

         Три причины, по которым отмена не срабатывала.

         Русская раскладка. Клавиша Z на ней даёт 'я', а сравнение шло
         с 'z'. Ctrl+Z не работал ровно у тех, кто печатает по-русски.
         Y на русской — 'н'.

         Чужая вкладка. Слушатель висит на window и живёт, пока открыт
         редактор. Врач уходил на «Развёртку», жал там Ctrl+Z — и вместе
         с мазком на развёртке молча откатывалась разметка КТ. Теперь
         этап сначала проверяет, что открыт именно он.

         Событие без key. У части событий (ввод через IME, автозаполнение)
         e.key не определён, и e.key.toLowerCase() ронял весь обработчик —
         вместе с остальными клавишами этапа. */
      function segStageActive() {
        const st = document.querySelector('.stage[data-stage="segment"]');
        return !!(st && st.classList.contains('active'));
      }
      function onKey(e) {
        if (!segStageActive()) return;
        const raw = e.key;
        ...
        const isZ = (k === 'z' || k === 'я');
        const isY = (k === 'y' || k === 'н');
        if (mod && isZ && e.shiftKey) { e.preventDefault(); redo(); return; }
        if (mod && isZ) { e.preventDefault(); undo(); return; }
        if (mod && isY) { e.preventDefault(); redo(); return; }
        if (raw === '[') { setSlice(AXIS[s.axis].get() - 1); return; }
        if (raw === ']') { setSlice(AXIS[s.axis].get() + 1); return; }
        // быстрый выбор инструмента (если не печатают в поле)
        if (mod || e.altKey) return;
        const ae = document.activeElement;
        if (ae && (ae.tagName === 'INPUT' || ae.tagName === 'TEXTAREA' || ae.isContentEditable)) return;
        // латиница и кириллица на одних и тех же физических клавишах
        if (k === 'b' || k === 'и') { setActiveTool('mucosa'); return; }
        if (k === 'e' || k === 'у') { setActiveTool('eraser'); }
      }
      ...
      function undo() {
        commitHistory();            // незакрытая правка сначала становится шагом
        if (!s.history.length) { _toast('Нечего отменять', 'info', 1800); return; }
        s.redo.push(_applyDelta(s.history.pop()));
      ...
      function redo() {
        ...
        s.history.push(_applyDelta(s.redo.pop()));
        if (s.history.length > HIST_MAX) s.history.shift();
        render(); if (s.r3d) s.r3d.markDirty();
      }
      ...
      /* ИНСТРУМЕНТ МОЖЕТ БЫТЬ НЕ ВЫБРАН ВООБЩЕ (s.tool === null).

         Раньше кисть включалась сама при открытии редактора и выключить
         её было нечем: любой клик по срезу что-то красил. Врач открывал
         этап, чтобы просто пролистать и посмотреть границу, а первый же
         клик по окну оставлял мазок. Отменяется это Ctrl+Z, но заметить
         надо успеть.

         Теперь ни один инструмент не активен, пока его не нажали, а
         повторное нажатие по активной кнопке отключает его обратно. Без
         инструмента клик по срезу только переносит перекрестье — то, что
         и нужно для осмотра. */
      function setActiveTool(t) {
        // повторное нажатие по активному — снять выделение
        if (t && t === s.tool) t = null;
        if (t && !_toolMeta[t]) t = null;
        s.tool = t;
        document.querySelectorAll('[data-segtool]').forEach((b) =>
          b.classList.toggle('active', !!t && b.dataset.segtool === t));
        const meta = t ? _toolMeta[t] : null;
        s.planes.forEach((p) => { if (p.canvas) p.canvas.style.cursor = meta ? meta.cursor : 'default'; });
        const hint = document.getElementById('segToolHint');
        if (hint) {
          hint.textContent = meta ? meta.hint
            : 'Инструмент не выбран: клик по срезу переносит перекрестье, колесо листает. ' +
              'Нажмите «Кисть» или «Ластик», чтобы править маску.';
        }
        // кружок кисти под курсором больше не нужен
        if (!t && s._brushCur) s._brushCur.style.display = 'none';
        /* Блок «размер/форма/воздух» имеет смысл только для кисти и ластика.

           Раньше он лишь приглушался классом seg-dim, а это чистая
           прозрачность: ползунок продолжал таскаться, галочка щёлкаться,
           кнопки формы нажиматься. Выглядело выключенным, работало
           включённым — худший из вариантов, потому что врач видит
           «недоступно», меняет значение и не понимает, почему на срезе
           ничего не изменилось.

           Поэтому гасим по-настоящему: disabled на самих элементах. Он
           заодно закрывает доступ с клавиатуры, чего pointer-events в CSS
           не умеет, и включает нативную отрисовку выключенного поля. */
        const bb = document.getElementById('segBrushBlock');
        if (bb) {
          const off = (t !== 'mucosa' && t !== 'eraser');
          bb.classList.toggle('seg-dim', off);
          bb.querySelectorAll('input, button, select').forEach((el) => {
            el.disabled = off;
          });
        }
      }
      ...
      /* Стартуем БЕЗ инструмента: вызов с null проставит подсказку и
         курсор для режима осмотра, но ничего не активирует. */
      setActiveTool(null);
      ...
      function onEdit3D(v) {
        ...
        if (!v) {
          if (_edit3dPushed) { commitHistory(); render(); if (s.r3d) s.r3d.markDirty(); }
          _edit3dPushed = false; s.lastPx = null; return;
        }
      ...
      function autoTidy() {
        ...
        if (!removed && !added) { _toast('Разметка модели уже чистая', 'info', 2200); return; }
        pushHistory();
        s.mask.set(m); render(); if (s.r3d) s.r3d.markDirty();
        _toast('Разметка модели автоматически подчищена. ' +
               'Ctrl+Z вернёт её в исходном виде.', 'ok', 4200);
      }
      autoTidy();
      ...
      return {
        ...
        undo, redo, resetToAuto() {
          ...
          if (s.r3d) s.r3d.markDirty();
          _toast('Все правки отменены — вернулась разметка модели', 'ok', 2500);
        },
        clean: cleanupNow,
        patch: patchHoles,
        closeFill,
        ...
        smooth: smoothMask,
        setCrosshair: (on) => { s.crosshair = !!on; render(); },
        ...
        /* Вкладку показали снова. installTabWatcher зовёт этот метод с
           самого начала, но в возвращаемом объекте его не было, и вызов
           молча гасился проверкой `editor.onShow &&`. Из-за этого холсты,
           посчитанные при нулевой ширине скрытого стейджа, оставались
           растянутыми до следующего ресайза окна. */
        onShow() {
          render();
          syncSlider();
          if (s.r3d && s.r3d.markDirty) s.r3d.markDirty();
        },
        dispose() {
          window.removeEventListener('mousemove', onMove);
          window.removeEventListener('mouseup', onUp);
  ...
  const SEG_ICONS = {
    ...
    // рамка с уголками — «выбрать область заново»
    region: [
      ['M3 6.5V4.5A1.5 1.5 0 014.5 3h2', 0],
      ['M11.5 3h2A1.5 1.5 0 0115 4.5v2', 0],
      ['M15 11.5v2a1.5 1.5 0 01-1.5 1.5h-2', 0],
      ['M6.5 15h-2A1.5 1.5 0 013 13.5v-2', 0],
      ['M6.75 6.75h4.5v4.5h-4.5z', 1],
    ],
    reset: [
  ...
  function injectCSS() {
    ...
    st.textContent = `
      ...
      .stage[data-stage="segment"] details#segGuideCard > summary::after {
        ...
        -webkit-mask:url("data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20width='10'%20height='14'%20viewBox='0%200%2010%2014'%20fill='none'%3E%3Cpath%20d='M7%202l-4%205%204%205'%20stroke='%23000'%20stroke-width='1.5'%20stroke-linecap='round'%20stroke-linejoin='round'/%3E%3C/svg%3E") center / 10px 14px no-repeat;
        mask:url("data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20width='10'%20height='14'%20viewBox='0%200%2010%2014'%20fill='none'%3E%3Cpath%20d='M7%202l-4%205%204%205'%20stroke='%23000'%20stroke-width='1.5'%20stroke-linecap='round'%20stroke-linejoin='round'/%3E%3C/svg%3E") center / 10px 14px no-repeat;
        transform:rotate(180deg);
        transition:transform .15s ease;
      }
      .stage[data-stage="segment"] details#segGuideCard[open] > summary::after { transform:rotate(270deg); }
      .stage[data-stage="segment"] details#segGuideCard[open] > summary { margin-bottom:8px; }
      /* Кнопка перехода — общий .btn-open-big, ровно как .data-next-btn
         на этапе «Модель». Своего фона здесь нет намеренно.

         Раньше стояло background:#1f6fd6 плоской заливкой. В светлой теме
         это перебивало градиент .light-theme .btn-open-big и давало
         другой синий, чем на соседнем этапе. В тёмной было хуже: фон
         переопределён на синий, а color и text-shadow остались от базы,
         то есть голубые буквы и голубой шеврон с ореолом поверх синего.

         Ниже — только раскладка, declaration в declaration та же, что у
         .data-next-btn в app.css. */
      ...
      /* pointer-events здесь страховка на случай, если в блок добавят
         элемент, которому disabled не поставить (ссылку, div-переключатель).
         Основную работу делает disabled — см. setActiveTool. */
      .seg-adv-block.seg-dim { opacity:.4; pointer-events:none; }
      .seg-adv-block.seg-dim label,
      .seg-adv-block.seg-dim .seg-mini { cursor:default; }
      .seg-adv-block input:disabled,
      .seg-adv-block button:disabled { cursor:default; }
      ...
      /* Пока работает модель, переключатель заблокирован.

         Невыбранная половина в обычном состоянии НЕ гасится: выбранную и
         так видно по подсветке, а серая соседка читалась как «сюда
         нельзя», хотя именно ею и переключаются. Гашение осталось ровно
         на тот случай, когда переключать действительно нельзя.

         Гасим ГРУППУ, а не каждую кнопку. Правило .seg-mini:disabled
         имело ту же специфичность, что .seg-mini.active, и стояло после
         неё — то есть перебивало подсветку выбранной половины, и на
         время работы модели выбранное выглядело выключённым. На
         контейнере такого конфликта нет: подсветка внутри сохраняется,
         видно и что заблокировано, и что именно выбрано. */
      .seg-seg.is-busy { opacity:.45; pointer-events:none; }
      ...
      /* Значок раскрытия — тот же шеврон, что у язычка панели на этапе
         «Развёртка»: path 'M7 2l-4 5 4 5', толщина 1.5, скруглённые
         концы и стык. Взят оттуда дословно.

         Рисуем маской, а не символом шрифта и не текстом: маска красится
         currentColor, поэтому значок сам следует за цветом заголовка и
         за сменой светлой/тёмной темы. Символ из шрифта такого не даёт —
         заголовок набран Orbitron, а угловые кавычки в нём и в запасном
         шрифте разной формы.

         Исходный path смотрит ВЛЕВО, отсюда развороты:
         закрыто 180° → вправо, открыто 270° → вниз. Разница те же 90°. */
      ...
      .seg-settings[open] > summary::after { transform:rotate(270deg); }
  ...
  window.Tab0Segment = { run: runInfer, exportNseg, downloadPair, downloadOBJ };
})();
