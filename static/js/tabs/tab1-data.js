...
(function () {
  ...
  function updatePanels() {
    ...
    if (stats) {
      if (hasMesh) {
        ...
        // Замкнутость: каждое ребро должно принадлежать ровно двум граням.
        // Считаем один раз при загрузке — на 200 тыс. граней это доли секунды.
        let openEdges = 0;
        try {
          const seen = new Map();
          const put = (a, b) => {
          ...
          seen.forEach(n => { if (n !== 2) openEdges++; });
        } catch (e) { openEdges = -1; }   // не смогли посчитать — не врём врачу

        const closedTxt = openEdges === 0
          ? '<span style="color:#22c55e">замкнута ✓</span>'
          : openEdges > 0
            ? '<span style="color:#DD8844">' + fmtN(openEdges) + ' открытых рёбер</span>'
            : '—';
        ...
        // бейдж «проверена» — только по замкнутости поверхности
        if (statsBadge) statsBadge.style.display = (openEdges === 0) ? '' : 'none';
    ...
    if (req && hasMesh) {
      req.innerHTML =
        ...
        '<div class="hint-text dim" style="margin-top:10px;font-size:11px;' +
          ...
          'Меш готов к следующему этапу — выделим слизистую оболочку.' +
        '</div>';
    }
  ...
  function injectResetCSS() {
    ...
    s.textContent = [
      ...
      '}',
      '.btn-reset:hover  { background: rgba(255, 159, 60, 0.10); }',
      '.btn-reset:active { background: rgba(255, 159, 60, 0.18); }',
