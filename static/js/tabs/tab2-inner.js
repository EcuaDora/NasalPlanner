/* ─── tabs/tab2-inner ──────────────────────────────────────────
   ...
   Mouse-семантика (заложена внутри редактора):
     ЛКМ        — применить активный инструмент (Кисть / Ластик)
...
(function () {
    ...
    if (emptyState) {
      emptyState.innerHTML = [
        ...
        '<div class="empty-sub" style="max-width:380px;line-height:1.5">',
          'Программа отделит слизистую оболочку от наружной ткани. ',
          'Результат вы проверите и поправите Кистью и Ластиком.',
        '</div>',
  ...
  const Editor = (function () {
    ...
    /* Подписи должны совпадать с тем, что написано на кнопках: инструмент
       назывался «Стереть» на кнопке и «ластиком» в инструкции — два имени
       у одного действия. Оставлено одно. */
    const TOOL_LABELS = { paint: 'Кисть', erase: 'Ластик',
                          pin: 'Точка реза',
                          cutline: 'Рез' };
    ...
    /* ─── ИСТОРИЯ ПРАВОК ────────────────────────────────────────────
       Снимок целиком — здесь это дёшево: маска это Uint8Array по граням,
       на 22 500 F выходит 22 КБ на шаг, весь стек около мегабайта.
       (На этапе 01 та же схема съедала гигабайт, потому что там маска —
       объём, а не поверхность.)

       Стек «Повторить» добавлен: раньше его не было вовсе — ни функции,
       ни кнопки, ни клавиши. Отменил лишнее — вернуть было нечем, только
       перерисовывать кистью. */
    const HIST_MAX = 50;

    function pushHistory(s) {
      s.history.push(new Uint8Array(s.mask));
      if (s.history.length > HIST_MAX) s.history.shift();
      if (s.redo) s.redo.length = 0;   // новая правка обрывает ветку «Повторить»
      syncHistoryButtons(s);
    }

    function undo(s) {
      /* СНАЧАЛА РЕЗ, ПОТОМ МАСКА.

         Рез живёт в своём состоянии и в историю маски не попадает — его
         снимает кнопка «Отменить» в карточке «Разрез». Из-за этого
         Ctrl+Z после реза откатывал ПРЕДЫДУЩИЙ мазок кистью, а меш
         оставался разрезанным. Не отказ, а тихое действие не над тем
         объектом — заметить это можно было только по пропавшей краске.

         Теперь порядок отмены совпадает с порядком действий: последним
         был рез — его и снимаем. */
      if (s.split || s.cutPoly) { resetCut(s); return; }
      if (!s.history.length) return;
      if (!s.redo) s.redo = [];
      s.redo.push(new Uint8Array(s.mask));
    ...
    function redo(s) {
      if (!s.redo || !s.redo.length) return;
      s.history.push(new Uint8Array(s.mask));
      if (s.history.length > HIST_MAX) s.history.shift();
      s.mask = s.redo.pop();
      refreshColors(s);
    ...
    /* Кнопка, которой нечего делать, должна выглядеть выключенной, а не
       молча не реагировать на клик. */
    function syncHistoryButtons(s) {
      if (!s.uiLeft) return;
      const u = s.uiLeft.querySelector('[data-act="undo"]');
      const r = s.uiLeft.querySelector('[data-act="redo"]');
      if (u) u.disabled = !s.history.length && !s.split && !s.cutPoly;
      if (r) r.disabled = !(s.redo && s.redo.length);
    }
    ...
    /* Кнопка «Отмена» теперь отвечает и за рез, значит её вид должен
       обновляться вместе с состоянием реза, а не только маски. */
    function updatePinUI(s) { syncSplitButtons(s); syncHistoryButtons(s); }
    ...
    function installInput(s) {
      ...
      /* ГОРЯЧИЕ КЛАВИШИ ЭТАПА.

         Три поправки, каждая закрывает свой способ «не сработать».

         Русская раскладка: Z даёт 'я', Y даёт 'н'. Сравнение шло только с
         латиницей, и Ctrl+Z не работал у всех, кто печатает по-русски.
         Обиднее всего, что в соседнем onSplitKey кириллица учтена — там
         про неё вспомнили, здесь нет.

         Чужая вкладка: слушатель висит на window и живёт, пока жив
         редактор. Ctrl+Z с другого этапа откатывал бы правку слизистой
         молча. Сначала проверяем, что открыт именно этот этап.

         Событие без key: у части событий (ввод через IME) e.key не
         определён, и e.key.toLowerCase() ронял весь обработчик разом. */
      const innerStageActive = () => {
        const st = document.querySelector('.stage[data-stage="inner"]');
        return !!(st && st.classList.contains('active'));
      };

      const onKey = e => {
        if (!innerStageActive()) return;
        ...
        if (e.ctrlKey || e.metaKey) {
          const isZ = (k === 'z' || k === 'я');
          const isY = (k === 'y' || k === 'н');
          if (isZ && e.shiftKey) { redo(s); e.preventDefault(); }
          else if (isZ)          { undo(s); e.preventDefault(); }
          else if (isY)          { redo(s); e.preventDefault(); }
          return;
        }
        if (e.altKey) return;
        // латиница и кириллица на одних и тех же физических клавишах
        if      (raw === 'Escape')            setTool(s, null);
        else if (k === 'b' || k === 'и')      setTool(s, 'paint');
        else if (k === 'e' || k === 'у')      setTool(s, 'erase');
        else if (k === 'l' || k === 'д')      { if (s.cutArmed) setTool(s, 'cutline'); }
      };
    ...
    function setTool(s, name) {
      ...
      if (hint) {
        hint.innerHTML = name
          ? '<b>' + TOOL_LABELS[name] + '.</b> ЛКМ — применить, ПКМ — повернуть модель.'
          : '<b>Инструмент не выбран.</b> ЛКМ — вращать, ПКМ или Shift+ЛКМ — сдвиг.';
      }
    ...
    function formatReportToast(r) {
      ...
      if (r.topology === 'disk') {
        ...
        parts.push(
          'В поверхности есть сквозной проход — развернуть её такой нельзя. ' +
          'Проведите Ластиком поперёк узкого места, чтобы разорвать его.'
        );
      }
    ...
    function installUI(s) {
      ...
      left.innerHTML = [
        '<div class="card">',
          ...
          '<div class="ep-tools">',
            '<button type="button" class="ep-tool active" data-tool="paint" title="Кисть: добавляет синее (клавиша B)">',
            ...
            '</button>',
            '<button type="button" class="ep-tool" data-tool="erase" title="Ластик: убирает синее (клавиша E)">',
              '<svg class="ep-tool-ico" width="16" height="16" viewBox="0 0 18 18" fill="none">',
          ...
          '<div class="ep-hint hint-text" data-mode-hint>',
            '<b>Кисть.</b> ЛКМ — красить, ПКМ — повернуть модель.',
          '</div>',
        '</div>',
        ...
        '<div class="card">',
          ...
          '<div class="ep-actions">',
            '<button type="button" class="ep-act" data-act="undo" title="Отменить последнее действие (Ctrl+Z). Если проведён рез — сначала снимется он">',
              ...
              'Отмена',
            '</button>',
            /* «Повторить» не было вовсе: ни кнопки, ни клавиши, ни стека.
               Отменил лишнее — вернуть нечем. */
            '<button type="button" class="ep-act" data-act="redo" title="Вернуть отменённое (Ctrl+Y)" disabled>',
              '<svg class="ep-act-ico" width="14" height="14" viewBox="0 0 18 18" fill="none">',
          ...
          '<button type="button" class="ep-rerun" data-act="rerun" ',
                  'title="Отменить все правки и вернуть выделение, которое предложила программа">',
            '<svg class="ep-act-ico" width="14" height="14" viewBox="0 0 18 18" fill="none">',
      ...
      right.innerHTML = [
        '<details class="card" id="innerGuideCard" open>',
          ...
          '<ol class="ep-steps">',
            '<li>Поверните модель <b>ПКМ</b>. <span class="accent">Синим</span> — ',
                 'то, что программа приняла за слизистую, ',
                 '<span class="ep-grey">серым</span> — наружная поверхность.</li>',
            '<li>Поправьте границу: <b>Кисть</b> добавляет синее, ',
                 '<b>Ластик</b> убирает.</li>',
            '<li>Не видно нужный участок изнутри — включите <b>Разрез</b> ',
                 'и проведите линию мышкой по модели.</li>',
            '<li>Линию ведите по воздушному просвету. Ткань пересекайте только ',
                 'в двух местах: на дне и там, где перегородка касается стенки. ',
                 'Удобнее это делать, повернув модель фронтально.</li>',
            '<li>Модель разделится надвое. <b>A</b> и <b>B</b> переключают, какую ',
                 'половину правите — клавиши <b>1</b> и <b>2</b> тоже работают.</li>',
            '<li><b>Ctrl+Z</b> отменяет последнее действие: сначала разрез, ',
                 'потом правки кистью. <b>Ctrl+Y</b> возвращает отменённое.</li>',
            '<li><b>Продолжить</b> — программа подчистит поверхность и откроет ',
                 '<b>«Зоны»</b>.</li>',
          '</ol>',
        '</details>',
      ...
      const onSplitKey = e => {
        ...
        if (typeof e.key !== 'string' || !e.key) return;   // события от IME приходят без key
    ...
    function injectCSS() {
      ...
      s.textContent = [
        ...
        /* ═══ Кнопки инструментов (Кисть / Ластик) ═══ */
        ...
        '  display: grid; grid-template-columns: 1fr 1fr; gap: 10px;',
        '}',
        /* Кнопок в сетке стало пять («Повторить» добавилась), и последняя
           оставалась одна в ряду с пустой клеткой справа. Растягиваем её
           на всю строку — заодно это честно: «Очистить» снимает всё
           выделение, и рядом с ней стоит такая же во всю ширину
           «Вернуть исходную маску». */
        '.stage[data-stage="inner"] .ep-actions .ep-act-danger {',
        '  grid-column: 1 / -1;',
        '}',
        ...
        /* Значок раскрытия — шеврон, как на этапах 01 и 02. Был залитый
           треугольник \u25B8. Рисуем маской, а не символом шрифта: маска
           красится currentColor и сама следует за цветом заголовка и за
           сменой темы. Исходный path смотрит ВЛЕВО, отсюда развороты:
           закрыто 180° вправо, открыто 270° вниз — те же 90°. */
        ...
        '}',
        '.stage[data-stage="inner"] details#innerGuideCard[open] > summary::after { transform: rotate(270deg); }',
    ...
    function install(viewer, fullMeshText, initialSelectionText) {
      ...
      const state = {
        ...
        history: [],
        redo: [],          // стека «Повторить» раньше не было вовсе
