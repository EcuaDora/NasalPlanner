...
(function () {
  ...
  function setupStaticUI() {
    ...
    if (empty) {
      ...
      empty.innerHTML = [
        '<div class="empty-title">Сегментация зон</div>',
  ...
  async function runZoneSeg() {
    ...
    if (!window.M || !window.M.V || !window.M.F || !window.M.nF) {
      _toast('Меш ещё не готов — сначала завершите этап 2.', 'err', 4000);
      _segInFlight = false;
      return;
    }
  ...
  //   Тот же приём, что в tab4.diagnoseState (_labelsEqual). Нужен, потому что
  //   tab4 после server-build переписывает window.M.V/F/zoneLabels НОВЫМИ
  //   typed-arrays — даже если сервер вернул ровно то же содержимое. Без
  //   этой проверки `editor.state.sourceV !== window.M.V` всегда срабатывает
  //   как ложно-положительный → disposeEditor → пересегментация на ровном
  //   месте, при простом возврате на tab3 без правок.
  //
  ...
  function syncActiveStates() {
    ...
    //      labels (длина === M.nF) — это значит tab4 server-build вернул новый меш
    //      со своими labels. SaddleSeg запускать НЕ надо: переустанавливаем editor
  ...
  window.addEventListener('tab:change', (e) => {
    ...
    // Универсальный авто-build: если tab4 cache отсутствует (`t4-built`
    // снят) — запускаем buildUnfold. Покрывает все пути:
    //   · после reinstall'а в tab3 (мы диспатчили zones:edit → disposeCache)
    //   · после ножниц (compactMeshFromDeletions диспатчит zones:edit → disposeCache)
    //   · после правок ползунков (emitChange диспатчит zones:edit)
    //   · первый заход на tab4 (cache никогда не было)
    // Если cache валиден — t4-built есть, ничего не делаем.
    setTimeout(() => {
      const stage = document.querySelector('.stage[data-stage="unfold"]');
      const tabActive = stage && stage.classList.contains('active');
      const built     = stage && stage.classList.contains('t4-built');
      if (tabActive && !built && window.Tab4 && typeof window.Tab4.build === 'function') {
        try { window.Tab4.build(); } catch (err) { console.warn('[tab3] Tab4.build:', err); }
      }
    }, 120);
    ...
    // Иначе deletedFaces проиндексированы под старую геометрию (например,
    // tab4 уже отстроил развёртку и переписал M.V/F) — compact даст мусор.
  ...
  const Editor = (function () {
    ...
    function installUI(s) {
      ...
      left.innerHTML = [
        '<div class="card">',
          '<div class="card-title">Сдвиг границ</div>',
          '<div class="hint-text dim" style="margin-bottom:10px;line-height:1.45">',
            'Двигайте границу между двумя соседними зонами по поверхности меша. ',
            '«0» — граница, которую построил алгоритм. Шкала — число шагов расширения ',
            'по графу смежности граней.',
          '</div>',
        ...
        '<div class="card">',
          ...
          '<div class="ep-actions">',
            '<button type="button" class="ep-act" data-act="reset">',
              ...
              'Сбросить',
            '</button>',
            '<button type="button" class="ep-act" data-act="scissors" ',
          ...
          '<button type="button" class="ep-act ep-act-sm" data-act="scissors-restore" ',
            ...
            '<svg class="ep-act-ico" width="14" height="14" viewBox="0 0 18 18" fill="none">',
              '<path d="M14.5 9a5.5 5.5 0 11-1.7-3.95M14.5 3v3.5h-3.5" ',
                    ...
                    'stroke-linecap="round" stroke-linejoin="round"/>',
            '</svg>',
            'Восстановить отрезанное',
          '</button>',
        '</div>',

        /* Файл — экспорт изменённого меша + сегментации (то, что
           уйдёт на этап 4: пара inner_zoned.obj + inner_zoned_segmentation.json) */
        '<div class="card">',
          '<div class="card-title">Файл<span class="badge">.OBJ + .JSON</span></div>',
          '<button type="button" class="btn-open ep-file-btn" data-act="export" ',
                  'title="Скачать изменённый меш и сегментацию">',
            '<svg width="14" height="14" viewBox="0 0 18 18" fill="none">',
        ...
        '<div class="card">',
          ...
          '<ol class="ep-steps">',
            '<li>Алгоритм разметил слизистую по трём зонам. Поверните меш ',
                 ', чтобы осмотреть границы.</li>',
            '<li>Если граница между зонами проходит не там, где нужно — ',
                 'двигайте ползунки.</li>',
            '<li><b>Сбросить</b> — оба ползунка в 0 (исходная разметка алгоритма).</li>',
            '<li><b>Ножницы</b> — обведите лассо ненужную часть меша: эти грани отрежутся и не уйдут на этап 4.</li>',
            '<li><b>Перейти к этапу 4</b> — построить интерактивную развёртку</li>',
          '</ol>',
          '<div class="ep-divider"></div>',
          '<div class="ep-ctrls-title">Управление</div>',
          '<div class="stat-row"><span class="stat-k">ЛКМ</span>',
            '<span class="stat-v">вращение</span></div>',
      ...
      //   3. switchTo('unfold').
      //   4. Если на tab4 нет CSS-класса `t4-built` (значит cache был
      //      disposed) — автоматически запускаем Tab4.build() через таймаут.
  ...
  function injectCSS() {
    ...
    s.textContent = [
      ...
      '}',
      '.stage[data-stage="zones"] .zn-slider-row input[type=range]:hover::-webkit-slider-thumb,',
      '.stage[data-stage="zones"] .ep-row input[type=range]:hover::-webkit-slider-thumb {',
      '  transform: scale(1.15);',
      '  box-shadow: 0 0 0 1px var(--cyan), 0 0 18px rgba(0,240,255,0.8);',
      '}',
      '.stage[data-stage="zones"] .zn-slider-row input[type=range]:active::-webkit-slider-thumb,',
      '.stage[data-stage="zones"] .ep-row input[type=range]:active::-webkit-slider-thumb {',
      ...
      '  background: rgba(74,158,255,0.06); border-color: rgba(74,158,255,0.3);',
      '}',
      ...
      '  box-shadow: 0 0 0 1px var(--cyan, #00f0ff), 0 0 12px rgba(0,240,255,0.35) inset;',
      '}',
      '.light-theme .stage[data-stage="zones"] .ep-act.zn-act-on {',
      '  background: rgba(74,158,255,0.12);',
      '  border-color: #4F7CDB; color: #4F7CDB;',
      '  box-shadow: 0 0 0 1px #4F7CDB, 0 0 10px rgba(79,124,219,0.25) inset;',
      '}',
